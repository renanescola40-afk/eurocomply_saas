...
describe('marketing attribution', () => {
  ...
  it('preserves first touch while updating last touch on a new acquisition signal', () => {
    window.history.replaceState({}, '', '/en/pricing?utm_source=google&utm_medium=cpc&utm_campaign=ai-governance');
    persistMarketingAttribution();
    window.history.replaceState({}, '', '/en/book-demo?utm_source=linkedin&utm_medium=paid-social&utm_campaign=article-50');
    persistMarketingAttribution();

    expect(getMarketingAttributionProperties('first_touch')).toMatchObject({
      utm_source: 'google', utm_medium: 'cpc', utm_campaign: 'ai-governance', landing_path: '/en/pricing', attribution_model: 'first_touch',
    });
    expect(getMarketingAttributionProperties('last_touch')).toMatchObject({
      utm_source: 'linkedin', utm_medium: 'paid-social', utm_campaign: 'article-50', landing_path: '/en/book-demo', attribution_model: 'last_touch',
    });
  });

  it('drops campaign values that are not bounded campaign tokens', () => {
    window.history.replaceState({}, '', '/en/pricing?utm_source=person%40example.com&utm_medium=paid-social&utm_campaign=ai%20governance');
    persistMarketingAttribution();
    const touch = JSON.parse(window.localStorage.getItem(marketingAttributionStorageKeys.firstTouch) || '{}') as Record<string, string>;
  ...
  it('allows bounded acquisition metadata while stripping PII-shaped string properties', () => {
    const sanitized = sanitizeAnalyticsProperties({
      utm_source: 'linkedin', utm_campaign: 'ai-governance', landing_path: '/en/pricing', attribution_model: 'last_touch',
      work_email: 'person@example.com', company_name: 'Example Ltd', free_text: 'should not leave the browser', count: 1,
    });
    expect(sanitized).toMatchObject({ utm_source: 'linkedin', utm_campaign: 'ai-governance', landing_path: '/en/pricing', attribution_model: 'last_touch', count: 1 });
