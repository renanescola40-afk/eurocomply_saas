...
describe('public commercial acquisition contract', () => {
  ...
  it('keeps the runtime acquisition taxonomy aligned with the prepared PostHog Actions', () => {
    const events = read('src/lib/analytics/events.ts');
    for (const event of [
      'landing_view','pricing_view','feature_view','trust_view','resource_view','cta_clicked','demo_started','demo_submitted',
      'document_downloaded','newsletter_subscribed','user_signed_up','checkout_started','checkout_completed','subscription_active',
