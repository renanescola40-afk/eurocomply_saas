...
describe('exact-SHA branch protection runtime proof', () => {
  ...
  it('keeps the protected workflow manual, exact-SHA-bound, and fail closed', () => {
    ...
    expect(generator).toContain("['dedicated-read-token', dedicatedToken]");
    expect(generator).toContain("['github-token', githubToken]");
    expect(generator).toContain("['public-read', '']");
    expect(generator).toContain("Authorization: \`Bearer \${candidate.token}\`");
    expect(generator).toContain("if (![401, 403, 404].includes(response.status))");
    expect(generator).toContain("requireBypassActors: true");
    expect(generator).toContain("Array.isArray(body?.bypass_actors)");
    expect(generator).toContain("Never combine fields from separate API snapshots.");
