...
vi.mock('@/lib/supabase/admin', () => ({
  tryCreateAdminClient: () => ({
    from: () => ({
      select: () => ({ eq: () => ({ maybeSingle: mocks.fetchMaybeSingle }) }),
      update: () => ({ eq: () => ({ eq: () => ({ select: () => ({ maybeSingle: mocks.updateMaybeSingle }) }) }) }),
    }),
  }),
}));
...
vi.mock('@/server/security/api-guards', () => ({
  ...
  parseJsonBodyWithZod: async (request: Request, options: { schema: { parse: (value: unknown) => unknown } }) => options.schema.parse(await request.json()),
...
function buildRequest(body = { action: 'approve', note: 'Looks good' }) {
...
describe('document approval API hardening', () => {
  beforeEach(() => {
    ...
    mocks.fetchMaybeSingle.mockResolvedValue({ data: { id: 'doc_a', organization_id: 'org_a', name: 'AI Policy', status: 'pending' }, error: null });
    mocks.updateMaybeSingle.mockResolvedValue({ data: { id: 'doc_a', organization_id: 'org_a', name: 'AI Policy', status: 'approved' }, error: null });
  });
  ...
  it('blocks requests rejected by the trusted mutation guard', async () => {
    mocks.requireTrustedMutation.mockResolvedValue(new Response(JSON.stringify({ error: 'invalid_origin' }), { status: 403, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } }));
  ...
  it('denies document IDs from another organization before update or notification', async () => {
    mocks.fetchMaybeSingle.mockResolvedValue({ data: { id: 'doc_b', organization_id: 'org_b', name: 'Other tenant policy', status: 'pending' }, error: null });

    const response = await POST(buildRequest(), params('doc_b'));
    const body = await response.json();

    expect(response.status).toBe(403);
    expect(body).toEqual({ error: 'organization_membership_required' });
    expect(mocks.updateMaybeSingle).not.toHaveBeenCalled();
    expect(mocks.createNotification).not.toHaveBeenCalled();
    expect(mocks.createAuditEvent).toHaveBeenCalledWith(expect.objectContaining({ action: 'document_approval_denied', organizationId: 'org_a', entityId: 'doc_b' }));
  });
});
