...
const progressWorkflow = readFileSync('.github/workflows/p0-progress.yml', 'utf8');
...
const PROGRESS_DYNAMIC_NAME = "name: ${{ needs.classify-source.outputs.authoritative == 'true' && 'Report P0 enterprise progress' || 'Stale P0 progress trigger (ignored)' }}";
...
test('stale or artifact-less P0 completions cannot reuse the authoritative downstream progress check name', () => {
  assert.match(progressWorkflow, /classify-source:/);
  assert.match(progressWorkflow, /PARENT_SHA: \$\{\{ github\.event\.workflow_run\.head_sha \|\| '' \}\}/);
  assert.match(progressWorkflow, /\[ "\$PARENT_SHA" != "\$WORKFLOW_SHA" \]/);
  assert.match(progressWorkflow, /actions\/runs\/\$\{PARENT_RUN_ID\}\/artifacts\?per_page=100/);
  assert.match(progressWorkflow, /p0-runtime-evidence-register-\$\{PARENT_SHA\}/);
  assert.ok(progressWorkflow.includes(PROGRESS_DYNAMIC_NAME));
  assert.match(progressWorkflow, /Stale P0 progress trigger \(ignored\)/);
  assert.match(progressWorkflow, /needs\.classify-source\.outputs\.authoritative == 'true'/);
});
