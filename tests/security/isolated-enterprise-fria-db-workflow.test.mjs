...
test('workflow is read-only and uses an ephemeral local database', () => {
  assert.match(workflow, /permissions:\n  contents: read/);
  assert.doesNotMatch(workflow, /contents: write|actions: write|pull_request_target/);
  assert.match(workflow, /supabase@2\.39\.2 start/);
  assert.match(workflow, /supabase@2\.39\.2 db reset --local --no-seed/);
  assert.match(workflow, /supabase@2\.39\.2 stop --no-backup/);
  assert.match(workflow, /persist-credentials: false/);
});

test('workflow retains only sanitized proof and never persists database credentials', () => {
  assert.match(workflow, /isolated-enterprise-fria-db-proof-\$\{\{ env\.TARGET_SHA \}\}/);
  assert.match(workflow, /retention-days: 90/);
  assert.match(workflow, /rm -f artifacts-supabase\.env/);
