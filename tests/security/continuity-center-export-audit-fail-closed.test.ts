...
const routePath = 'src/app/api/continuity-center/export/route.ts';

describe('continuity center export audit persistence', () => {
  it('does not return the export when its audit event is not persisted', () => {
    ...
    expect(source).toContain("console.error('[continuity-center] export_audit_unavailable')");
    ...
    const downloadIndex = source.indexOf('return noStoreDownload(');
