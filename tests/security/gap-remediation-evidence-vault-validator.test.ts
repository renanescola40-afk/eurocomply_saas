...
describe('Gap remediation Evidence Vault runtime validator', () => {
  it('tracks the canonical organization-scoped Evidence Vault policy contract', () => {
    ...
    expect(validator).toContain('authenticated must not have hard DELETE privilege on Evidence Vault metadata');
    expect(validator).toContain('authenticated Evidence Vault storage UPDATE/DELETE policy remains active');
  });
