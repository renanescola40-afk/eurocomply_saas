...
describe('customer-facing claims guard', () => {
  ...
  it('keeps corrected customer surfaces evidence-safe and on-brand', () => {
    ...
    const productShell = fs.readFileSync(
      path.join(rootDir, 'src/components/dashboard/authenticated-product-shell.tsx'),
      'utf8',
    );
    ...
    expect(retentionCenter).toContain('AuthenticatedProductShell');
    expect(retentionCenter).not.toContain('DashboardCommandNavigation');
    expect(productShell).not.toContain('EuroComply');
    expect(productShell).toContain('RISCK COMPLY');
