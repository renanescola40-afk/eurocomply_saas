...
describe('recovery resilience promotion megapack',()=>{
 ...
 it('emits exact-SHA promotable rollback and restore evidence',()=>{ for(const token of ["controlsVerified: ['REC-01', 'REC-02', 'REC-03', 'REC-04']",'observedSha',"runId: env('GITHUB_RUN_ID')",'containsSensitiveValues: false',"redirect: 'error'"]) expect(rollback).toContain(token); for(const token of ["controlsVerified: ['REC-05', 'REC-06', 'REC-07', 'REC-08', 'REC-09', 'REC-10']",'singleDescriptorInspection: !ephemeralMode','logicalBackupFilesDeleted: true','inspectLogicalBackup([rolesDumpPath, schemaDumpPath, dataDumpPath])','rpoMeasured','rtoMeasured','containsSensitiveValues: false']) expect(restore).toContain(token); });
