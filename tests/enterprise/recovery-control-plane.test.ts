...
const legacyBuilder = readFileSync('scripts/enterprise/build-recovery-evidence.mjs', 'utf8');
...
describe('enterprise recovery control plane', () => {
  ...
  it('keeps the retired synthetic recovery builder fail-closed', () => {
    expect(legacyBuilder).toContain('retired_synthetic_recovery_evidence_builder');
    expect(legacyBuilder).toContain('process.exitCode = 1');
    expect(legacyBuilder).not.toContain('rollback-validation.v2');
    expect(legacyBuilder).not.toContain('rollback-source.json');
    expect(legacyBuilder).not.toContain('recovery-source.json');
  });
});
