...
describe('upload scanner P0 aggregation', () => {
  ...
  it('keeps direct PR validation read-only and reserves promotion for workflow handoff', () => {
    ...
    expect(workflow).not.toContain("github.event.workflow.path == '.github/workflows/upload-security-ci.yml'");
    expect(workflow).not.toContain("github.event.workflow.path == '.github/workflows/branch-protection-runtime-proof.yml'");
