...
const approvedIds = [
  ...
  'SEC-05', 'SEC-06',
  'REL-02', 'REL-03', 'REL-04', 'REL-05', 'REL-06',
];
...
describe('enterprise evidence override guard', () => {
  it('accepts only the committed approved control and evidence pairs', () => {
    ...
    expect(Object.keys(APPROVED_EVIDENCE_OVERRIDES)).toEqual(approvedIds);
  });
  ...
  it('rejects an arbitrary repository JSON path', () => {
    ...
    expect(validateEvidenceOverrides(controls, document)).toContain('TRU-01 evidence.path is not approved');
  });

  it('rejects an unsupported or omitted exact-SHA check', () => {
    const unsupported = clone(committedOverrides);
    const runtimeEntry = unsupported.overrides.find((entry: { controlId: string }) => entry.controlId === 'REL-02');
    runtimeEntry.evidence.check = 'build';
    expect(validateEvidenceOverrides(controls, unsupported)).toContain('REL-02 evidence.check is not approved');

    const omitted = clone(committedOverrides);
    delete omitted.overrides[0].evidence.check;
    expect(validateEvidenceOverrides(controls, omitted)).toContain('TRU-01 evidence fields must be exactly path and check');
  });
  ...
  it('rejects added, removed, duplicate, unknown, or policy-changing entries', () => {
    ...
    expect(validateEvidenceOverrides(controls, added)).toEqual(expect.arrayContaining([
      `evidence overrides must contain exactly ${approvedIds.length} approved entries`,
      'evidence override is not approved: REL-01',
    ]));

    const removed = clone(committedOverrides);
    removed.overrides = removed.overrides.filter((entry: { controlId: string }) => entry.controlId !== 'REL-06');
    expect(validateEvidenceOverrides(controls, removed)).toEqual(expect.arrayContaining([
      `evidence overrides must contain exactly ${approvedIds.length} approved entries`,
      'approved evidence override is missing: REL-06',
    ]));
    ...
    expect(validateEvidenceOverrides(controls, policyChanging)).toContain('each evidence override must contain exactly controlId and evidence');
  });
});
