...
function writeJson(root, path, document) {
  const absolute = join(root, path);
  mkdirSync(join(absolute, '..'), { recursive: true });
  writeFileSync(absolute, `${JSON.stringify(document, null, 2)}\n`);
}

function writeText(root, path, value = 'evidence') {
  const absolute = join(root, path);
  mkdirSync(join(absolute, '..'), { recursive: true });
  writeFileSync(absolute, value);
}

function createFixture({ includeAcceptedReview = false, reviewSha = 'sha-main' } = {}) {
...
describe('legal truth audit', () => {
  ...
  it('accepts a complete, current, exact-SHA signed review record', () => {
    const root = createFixture({ includeAcceptedReview: true });
    const report = auditLegalTruth({
      root,
      sourceSha: 'sha-main',
      now: new Date('2026-07-30T12:00:00.000Z'),
  ...
  it('rejects a review that is otherwise complete but bound to a different SHA', () => {
    const root = createFixture({ includeAcceptedReview: true, reviewSha: 'different-sha' });
    const report = auditLegalTruth({
      root,
      sourceSha: 'sha-main',
      now: new Date('2026-07-30T12:00:00.000Z'),
  ...
  it('flags article rows that grant non-human-review status while review evidence is required', () => {
    const root = createFixture();
    writeJson(root, 'docs/compliance/article-function-evidence-registry.v1.json', {
      regulation: 'Regulation (EU) 2024/1689, as amended by Regulation (EU) 2026/1744',
      entries: [
        {
          article: 'Article 5',
          status: 'IMPLEMENTED_EVIDENCE_REVALIDATION_REQUIRED',
          humanReviewEvidence: [
