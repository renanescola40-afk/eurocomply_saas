...
VALUES (
  '20260922203500',
  ARRAY['reconcile backend-only team membership role-change and removal RPCs'],
  'reconcile_team_membership_runtime_rpcs'
)
...
begin
  ...
  where n.nspname='public'
    and p.proname='change_organization_member_role_atomic'
    and pg_get_function_identity_arguments(p.oid)='p_organization_id uuid, p_member_id uuid, p_expected_role text, p_next_role text';

  if v_oid is null then
    raise exception 'change_organization_member_role_atomic missing after reconciliation';
  end if;
  if not (select prosecdef from pg_proc where oid=v_oid) then
    raise exception 'change_organization_member_role_atomic must be SECURITY DEFINER';
  end if;
  if has_function_privilege('anon', v_oid, 'execute')
     or has_function_privilege('authenticated', v_oid, 'execute')
     or has_function_privilege('public', v_oid, 'execute')
     or not has_function_privilege('service_role', v_oid, 'execute') then
    raise exception 'change_organization_member_role_atomic ACL mismatch';
  end if;
  ...
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname='public'
    and p.proname='remove_organization_member_atomic'
    and pg_get_function_identity_arguments(p.oid)='p_organization_id uuid, p_member_id uuid, p_expected_user_id uuid, p_expected_role text';

  if v_oid is null then
    raise exception 'remove_organization_member_atomic missing after reconciliation';
  end if;
  if not (select prosecdef from pg_proc where oid=v_oid) then
    raise exception 'remove_organization_member_atomic must be SECURITY DEFINER';
  end if;
  if has_function_privilege('anon', v_oid, 'execute')
     or has_function_privilege('authenticated', v_oid, 'execute')
     or has_function_privilege('public', v_oid, 'execute')
     or not has_function_privilege('service_role', v_oid, 'execute') then
    raise exception 'remove_organization_member_atomic ACL mismatch';
  end if;
end $$;
