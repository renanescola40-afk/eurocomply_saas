...
create table if not exists public.organization_add_ons (
  ...
  constraint organization_add_ons_status_check
    check (status in ('inactive', 'active', 'trialing', 'past_due', 'cancelled')),
...
drop policy if exists "organization members can read add-ons"
  on public.organization_add_ons;
create policy "organization members can read add-ons"
  on public.organization_add_ons
  for select
  to authenticated
  using (
...
-- Add-on state is billing-provider owned. Browser roles may read their tenant's
-- state through RLS but cannot mutate billing authority directly.
revoke all on table public.organization_add_ons from anon;
revoke insert, update, delete, truncate, references, trigger
  on table public.organization_add_ons from authenticated;
...
drop trigger if exists organization_add_ons_set_updated_at
  on public.organization_add_ons;
create trigger organization_add_ons_set_updated_at
  before update on public.organization_add_ons
  for each row
  execute function public.touch_organization_add_on_updated_at();

comment on table public.organization_add_ons is
  'Canonical organization-scoped Stripe add-on state. Browser writes are denied; service-role billing flows own mutations.';
