...
-- Reconcile the production schema with the reviewed server contracts that are
-- already deployed by the application. This migration is intentionally
-- idempotent, fail-closed, and contains no destructive data rewrite.
...
-- ---------------------------------------------------------------------------
-- Tasks were left with only live_rls_* policies in production. Create the
-- canonical chain first so cleanup cannot make the table inaccessible.
...
-- AI incidents are server-written. Remove legacy member-wide write policies and
-- preserve tenant-scoped reads only.
...
-- Remove legacy AI-system policies whose broader role vocabulary can OR with
-- the canonical private-helper policies.
...
-- Billing state is backend-owned. A legacy FOR ALL owner policy defeats the
-- backend-only canonical INSERT/UPDATE/DELETE policies because RLS policies are
-- permissive by default and are combined permissively.
...
-- Every live_rls_* policy is a temporary validation artifact. Remove them only
-- after canonical policies above (and the already-deployed canonical policies
-- on notifications, monitoring, onboarding, audit events, invitations and
-- regulatory updates) are present.
...
-- Temporary helper RPCs must not survive production validation.
