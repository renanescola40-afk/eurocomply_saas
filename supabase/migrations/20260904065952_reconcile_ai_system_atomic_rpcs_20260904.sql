...
begin
  ...
  insert into public.ai_systems (
    organization_id,created_by,name,owner_team,category,country_market,processed_data,vendor_name,model_name,use_case,role,lifecycle_status,risk_domain,uses_personal_data,interacts_with_people,generates_content,biometric_identification,manipulative_or_exploitative,risk_level,classification_summary,obligations,next_actions,last_reassessed_at
  ) values (
    p_organization_id,p_actor_user_id,trim(p_system ->> 'name'),nullif(trim(coalesce(p_system ->> 'owner_team','')),''),nullif(trim(coalesce(p_system ->> 'category','')),''),nullif(trim(coalesce(p_system ->> 'country_market','')),''),nullif(trim(coalesce(p_system ->> 'processed_data','')),''),nullif(trim(coalesce(p_system ->> 'vendor_name','')),''),nullif(trim(coalesce(p_system ->> 'model_name','')),''),trim(p_system ->> 'use_case'),lower(trim(p_system ->> 'role')),lower(trim(p_system ->> 'lifecycle_status')),trim(p_system ->> 'risk_domain'),(p_system ->> 'uses_personal_data')::boolean,(p_system ->> 'interacts_with_people')::boolean,(p_system ->> 'generates_content')::boolean,(p_system ->> 'biometric_identification')::boolean,(p_system ->> 'manipulative_or_exploitative')::boolean,lower(trim(p_system ->> 'risk_level')),p_system ->> 'classification_summary',p_system -> 'obligations',p_system -> 'next_actions',now()
  ) returning * into v_created;
  insert into public.ai_system_history(ai_system_id,organization_id,actor_user_id,action,snapshot)
  values(v_created.id,v_created.organization_id,p_actor_user_id,'created',jsonb_build_object('name',v_created.name,'riskLevel',v_created.risk_level,'lifecycleStatus',v_created.lifecycle_status,'riskDomain',v_created.risk_domain,'createdAt',v_created.created_at));
...
create or replace function public.reassess_ai_system_atomic(
  p_system_id uuid,p_organization_id uuid,p_expected_updated_at timestamptz,p_actor_user_id uuid,p_patch jsonb
)
returns table(outcome text, system jsonb)
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(p_patch -> 'name') <> 'string'
    ...
    or jsonb_typeof(p_patch -> 'next_actions') <> 'array' then
    return query select 'invalid_input'::text,null::jsonb; return;
  end if;
  select s.* into v_current from public.ai_systems s where s.id=p_system_id and s.organization_id=p_organization_id for update;
  if not found then return query select 'not_found'::text,null::jsonb; return; end if;
  if v_current.updated_at is distinct from p_expected_updated_at then return query select 'state_changed'::text,null::jsonb; return; end if;
  update public.ai_systems s set
    ...
    use_case=trim(p_patch ->> 'use_case'),role=lower(trim(p_patch ->> 'role')),lifecycle_status=lower(trim(p_patch ->> 'lifecycle_status')),risk_domain=trim(p_patch ->> 'risk_domain'),
    uses_personal_data=(p_patch ->> 'uses_personal_data')::boolean,interacts_with_people=(p_patch ->> 'interacts_with_people')::boolean,generates_content=(p_patch ->> 'generates_content')::boolean,biometric_identification=(p_patch ->> 'biometric_identification')::boolean,manipulative_or_exploitative=(p_patch ->> 'manipulative_or_exploitative')::boolean,
    risk_level=lower(trim(p_patch ->> 'risk_level')),classification_summary=p_patch ->> 'classification_summary',obligations=p_patch -> 'obligations',next_actions=p_patch -> 'next_actions',last_reassessed_at=now()
  ...
  if not found then return query select 'state_changed'::text,null::jsonb; return; end if;
  insert into public.ai_system_history(ai_system_id,organization_id,actor_user_id,action,snapshot)
  values(v_updated.id,v_updated.organization_id,p_actor_user_id,'reassessed',jsonb_build_object('name',v_updated.name,'riskLevel',v_updated.risk_level,'lifecycleStatus',v_updated.lifecycle_status,'riskDomain',v_updated.risk_domain,'updatedAt',v_updated.updated_at));
  return query select 'updated'::text,to_jsonb(v_updated);
end;$$;
revoke all on function public.reassess_ai_system_atomic(uuid,uuid,timestamptz,uuid,jsonb) from public, anon, authenticated;
