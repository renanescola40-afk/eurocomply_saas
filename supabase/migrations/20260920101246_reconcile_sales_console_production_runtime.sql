-- Reconcile the internal Sales Console runtime in production.
-- Additive/idempotent: creates the missing lead tables, expands the MVP schema,
-- keeps direct client access revoked, and grants only backend service_role access.
