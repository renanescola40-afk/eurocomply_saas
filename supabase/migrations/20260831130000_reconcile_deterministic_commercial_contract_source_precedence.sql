...
-- Forward-only V25 reconciliation after the deterministic V24 function body was
-- applied through a provider operation that recorded a non-canonical migration
-- ledger version. Do not repair, rewrite or delete existing migration history.
-- Re-assert the deterministic commercial authority under a canonical forward
-- version strictly later than the observed Production migration head.
...
comment on function app_private.resolve_commercial_plan(uuid) is
  'V25 canonical forward reconciliation: deterministic fail-closed commercial plan resolver, highest priority then immutable source UUID, with exact processed Stripe LIVE fallback.';
