-- Enterprise tenant licensing core.
--
-- This migration adds contract-backed entitlements, explicit seat types and
-- serialized seat reservation. All material writes remain backend-only and are
-- exposed through service-role RPCs with fixed search paths.
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'organization_members_seat_type_check'
  ) then
    alter table public.organization_members
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'invitations_seat_type_check'
  ) then
    alter table public.invitations
      add constraint invitations_seat_type_check
      check (seat_type in ('full', 'participant', 'viewer'));
  end if;
end;
$$;
...
create index if not exists enterprise_contracts_status_renewal_idx
  on public.enterprise_contracts (status, renews_at);
...
create table if not exists public.organization_usage (
  ...
  constraint organization_usage_non_negative_check check (
    ...
    and viewers >= 0
    and active_admins >= 0
    and pending_invitations >= 0
    and ai_systems >= 0
    and legal_entities >= 0
    and storage_bytes >= 0
    and api_requests >= 0
    and scim_operations >= 0
    and csv_imports >= 0
  ),
  constraint organization_usage_seat_sum_check check (
...
create table if not exists public.platform_admins (
  ...
  constraint platform_admins_role_check check (
    role in ('platform_owner', 'platform_admin', 'platform_billing', 'platform_support', 'platform_security', 'platform_auditor')
  ),
...
-- Existing tenants receive a deliberately non-breaking compatibility contract.
-- Platform operators can replace these values with negotiated limits before an
-- Enterprise contract is activated.
...
select
  ...
  'Backfilled compatibility contract. Review and replace before commercial enforcement.'
...
left join lateral (
  select
    count(*) filter (where coalesce(member.status, 'active') = 'active')::integer as active_members,
    count(*) filter (
      where coalesce(member.status, 'active') = 'active'
        and coalesce(member.seat_type, 'full') = 'full'
    )::integer as full_users,
    count(*) filter (
      where coalesce(member.status, 'active') = 'active'
...
select
  ...
  contract.full_user_limit,
  contract.participant_limit,
  contract.viewer_limit,
  contract.admin_limit,
  contract.legal_entity_limit,
  contract.ai_system_limit,
  contract.storage_limit_bytes,
  contract.audit_retention_days,
  contract.sso_enabled,
  contract.scim_enabled,
  contract.api_enabled,
  contract.webhooks_enabled,
  contract.custom_roles_enabled,
  contract.advanced_reports_enabled,
  contract.priority_support_enabled,
...
select
  organization.id,
  count(member.id) filter (where coalesce(member.status, 'active') = 'active')::integer,
  ...
  count(member.id) filter (
    where coalesce(member.status, 'active') = 'active'
      and member.seat_type = 'participant'
  )::integer,
  count(member.id) filter (
    where coalesce(member.status, 'active') = 'active'
      and member.seat_type = 'viewer'
  )::integer,
  count(member.id) filter (
    where coalesce(member.status, 'active') = 'active'
      and lower(coalesce(member.role, '')) in ('owner', 'admin')
...
create or replace function public.resolve_organization_entitlements(
...
begin
  if p_organization_id is null then
    return query select
      'invalid_input'::text, null::uuid, null::text,
  ...
  if not found then
    return query select
      'contract_missing'::text, null::uuid, null::text,
      null::integer, null::integer, null::integer, null::integer, null::integer,
      null::integer, null::integer, null::integer, null::integer, null::integer,
      null::boolean, null::boolean, null::boolean, null::boolean;
    return;
  ...
  if not found then
    return query select
      'entitlements_missing'::text, v_contract.id, v_contract.status,
      null::integer, null::integer, null::integer, null::integer, null::integer,
      null::integer, null::integer, null::integer, null::integer, null::integer,
      null::boolean, null::boolean, null::boolean, null::boolean;
    return;
  ...
  if not found then
    return query select
      'usage_missing'::text, v_contract.id, v_contract.status,
...
create or replace function public.reserve_organization_seat_atomic(
  p_organization_id uuid,
  p_user_id uuid,
  p_role text,
  p_seat_type text,
...
declare
  ...
  v_current_seat_usage integer;
  v_current_active_members integer;
...
begin
  ...
  insert into public.enterprise_seat_operations (
    organization_id, idempotency_key, operation, source, actor_user_id,
    target_user_id, requested_seat_type
  ) values (
    p_organization_id, p_idempotency_key, 'reserve', v_source,
    p_actor_user_id, p_user_id, v_seat_type
  ...
  if not found then
    update public.enterprise_seat_operations
      set outcome = 'contract_missing', completed_at = now()
      where id = v_operation_id;
    return query select 'contract_missing'::text, null::uuid, null::text, null::text, 0, 0, 0;
    return;
  end if;

  if v_contract.status <> 'active' then
    update public.enterprise_seat_operations
      set outcome = 'contract_not_active', completed_at = now(),
          metadata = jsonb_build_object('contract_status', v_contract.status)
      where id = v_operation_id;
    return query select 'contract_not_active'::text, null::uuid, null::text, null::text, 0, 0, 0;
    return;
  end if;
  ...
  if not found then
    update public.enterprise_seat_operations
      set outcome = 'entitlements_missing', completed_at = now()
      where id = v_operation_id;
    return query select 'entitlements_missing'::text, null::uuid, null::text, null::text, 0, 0, 0;
    return;
  end if;
  ...
  -- The usage row serializes every seat-consuming path for this organization.
  -- Recount under that lock so stale counters cannot authorize an extra seat.
  select
  ...
  if v_previous_active and v_previous_seat_type = v_seat_type then
    update public.enterprise_seat_operations
      set outcome = 'already_active', membership_id = v_membership.id,
          completed_at = now(),
          metadata = jsonb_build_object(
            'active_members', v_usage.active_members,
            'seat_usage', case v_seat_type
              when 'full' then v_usage.full_users
              when 'participant' then v_usage.participants
              else v_usage.viewers
            end,
            'seat_limit', case v_seat_type
  ...
  v_current_active_members := v_usage.active_members;
  v_current_seat_usage := case v_seat_type
    when 'full' then v_usage.full_users
    when 'participant' then v_usage.participants
    else v_usage.viewers
  end;
  v_current_admins := v_usage.active_admins;
  v_is_admin := v_role in ('owner', 'admin');

  if not v_previous_active and v_current_active_members >= v_member_limit then
    update public.enterprise_seat_operations
      set outcome = 'member_limit_reached', completed_at = now(),
          metadata = jsonb_build_object(
            'active_members', v_current_active_members,
            'seat_usage', v_current_seat_usage,
            'seat_limit', v_seat_limit
          )
      where id = v_operation_id;
    return query select 'member_limit_reached'::text, null::uuid, null::text, v_seat_type,
      v_current_active_members, v_current_seat_usage, v_seat_limit;
    return;
  end if;

  if (not v_previous_active or v_previous_seat_type <> v_seat_type)
    and v_current_seat_usage >= v_seat_limit then
    update public.enterprise_seat_operations
      set outcome = 'seat_limit_reached', completed_at = now(),
          metadata = jsonb_build_object(
            'active_members', v_current_active_members,
            'seat_usage', v_current_seat_usage,
            'seat_limit', v_seat_limit
      ...
      where id = v_operation_id;
    return query select 'seat_limit_reached'::text, null::uuid, null::text, v_seat_type,
      v_current_active_members, v_current_seat_usage, v_seat_limit;
    return;
  end if;
  ...
  if v_is_admin
    ...
    and v_current_admins >= least(v_contract.admin_limit, v_entitlement.admin_limit) then
    update public.enterprise_seat_operations
      set outcome = 'admin_limit_reached', completed_at = now(),
          metadata = jsonb_build_object(
            'active_members', v_current_active_members,
            'seat_usage', v_current_admins,
            'seat_limit', least(v_contract.admin_limit, v_entitlement.admin_limit)
          )
      where id = v_operation_id;
    return query select 'admin_limit_reached'::text, null::uuid, null::text, v_seat_type,
      v_current_active_members, v_current_admins,
      least(v_contract.admin_limit, v_entitlement.admin_limit);
    return;
  end if;
  ...
  insert into public.organization_members (
    ...
    p_organization_id, p_user_id, v_role, v_seat_type, 'active', now()
  )
  ...
  insert into public.audit_logs (
    ...
    p_actor_user_id,
    case
      when v_previous_active then 'enterprise.member_seat_type_changed'
      else 'enterprise.seat_reserved'
    end,
    'organization_member',
    ...
    jsonb_build_object(
      ...
      'role', v_role,
      'previous_seat_type', v_previous_seat_type
    )
  );

  update public.enterprise_seat_operations
    set outcome = case when v_previous_active then 'seat_changed' else 'reserved' end,
  ...
  return query select
    case when v_previous_active then 'seat_changed'::text else 'reserved'::text end,
    v_membership.id,
    v_membership.role,
    v_membership.seat_type,
    v_usage.active_members,
...
begin
  ...
  if v_source not in ('invitation', 'csv', 'scim', 'sso', 'api', 'platform', 'admin', 'reactivation') then
    return query select 'invalid_source'::text, null::uuid, null::text, 0;
    return;
  end if;
  ...
  insert into public.enterprise_seat_operations (
    ...
    p_organization_id, p_idempotency_key, 'release', v_source, p_actor_user_id
  )
  on conflict (organization_id, idempotency_key) do nothing
  returning id into v_operation_id;

  if v_operation_id is null then
    return query select 'duplicate'::text, null::uuid, null::text, 0;
    return;
  end if;
  ...
  if not found then
    update public.enterprise_seat_operations
      set outcome = 'not_found', completed_at = now()
      where id = v_operation_id;
    return query select 'not_found'::text, p_member_id, null::text, 0;
    return;
  end if;

  if v_membership.status <> 'active' then
    update public.enterprise_seat_operations
      set outcome = 'already_released', membership_id = v_membership.id, completed_at = now()
      where id = v_operation_id;
    return query select 'already_released'::text, v_membership.id, v_membership.seat_type,
      (select count(*)::integer from public.organization_members where organization_id = p_organization_id and status = 'active');
    return;
  end if;

  update public.organization_members set
    status = case when v_source = 'scim' then 'deprovisioned' else 'suspended' end,
    updated_at = now()
  ...
  update public.enterprise_seat_operations
    set outcome = 'released', membership_id = v_membership.id, completed_at = now(),
        metadata = jsonb_build_object('active_members', v_usage.active_members)
    where id = v_operation_id;

  return query select 'released'::text, v_membership.id, v_membership.seat_type, v_usage.active_members;
end;
$$;

create or replace function public.reconcile_organization_usage_atomic(
  p_organization_id uuid,
  p_actor_user_id uuid
)
returns table (
...
begin
  if p_organization_id is null or p_actor_user_id is null then
    return query select 'invalid_input'::text, 0, 0, 0, 0, 0, 0;
    return;
  end if;
  ...
  select count(*)::integer into v_usage.pending_invitations
  from public.invitations as invitation
  ...
  update public.organization_usage set
    ...
    viewers = v_usage.viewers,
    active_admins = v_usage.active_admins,
    pending_invitations = v_usage.pending_invitations,
    last_reconciled_at = now(),
    updated_at = now()
  where organization_id = p_organization_id;
  ...
  return query select
    ...
    v_usage.viewers,
    v_usage.active_admins,
    v_usage.pending_invitations;
end;
$$;

-- Replace invitation acceptance so invitation, seat reservation, membership and
-- invite consumption remain in one transaction and the same organization lock.
...
revoke all on function public.resolve_organization_entitlements(uuid) from public, anon, authenticated;
revoke all on function public.reserve_organization_seat_atomic(uuid, uuid, text, text, uuid, text, text) from public, anon, authenticated;
revoke all on function public.release_organization_seat_atomic(uuid, uuid, uuid, text, text) from public, anon, authenticated;
revoke all on function public.reconcile_organization_usage_atomic(uuid, uuid) from public, anon, authenticated;
revoke all on function public.accept_organization_invitation_atomic(text, uuid, text) from public, anon, authenticated;

grant execute on function public.resolve_organization_entitlements(uuid) to service_role;
grant execute on function public.reserve_organization_seat_atomic(uuid, uuid, text, text, uuid, text, text) to service_role;
grant execute on function public.release_organization_seat_atomic(uuid, uuid, uuid, text, text) to service_role;
grant execute on function public.reconcile_organization_usage_atomic(uuid, uuid) to service_role;
