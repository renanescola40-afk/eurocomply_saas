...
returns table (outcome text,identity_id uuid,external_id text,user_id uuid,email text,role text,seat_type text,active boolean,created_at timestamptz,updated_at timestamptz)
language sql security definer set search_path=pg_catalog
as $$
  select 'resolved'::text,identity.id,identity.external_id,identity.user_id,identity.email,identity.role,
