...
create table if not exists public.qualified_reviewer_attestations (
  ...
  foreign key (assignment_id, organization_id) references public.qualified_review_assignments(id, organization_id) on delete cascade,
  foreign key (reviewer_id, organization_id) references public.qualified_reviewers(id, organization_id) on delete cascade,
  check ((independence_confirmed and nullif(trim(conflict_details), '') is null) or not independence_confirmed)
);
...
begin
  ...
  from public.qualified_reviewer_invites
  ...
  if p_session_expires_at <= now() or p_session_expires_at > now() + interval '12 hours' then
    raise exception 'session_expiry_invalid';
  end if;

  update public.qualified_reviewer_invites set accepted_at = now() where id = v_invite.id;
  update public.qualified_reviewer_sessions
    set revoked_at = now()
    where assignment_id = v_invite.assignment_id and reviewer_id = v_invite.reviewer_id and revoked_at is null;
...
create or replace function public.expire_qualified_reviewer_access()
returns table (expired_invites integer, expired_sessions integer)
language plpgsql
security definer
set search_path = public
as $$
declare
  ...
  v_sessions integer;
begin
  update public.qualified_reviewer_invites set revoked_at = now()
  where accepted_at is null and revoked_at is null and expires_at <= now();
  get diagnostics v_invites = row_count;

  update public.qualified_reviewer_sessions set revoked_at = now()
  where revoked_at is null and expires_at <= now();
  get diagnostics v_sessions = row_count;

  return query select v_invites, v_sessions;
end;
$$;

revoke all on function public.accept_qualified_reviewer_invite(text, text, timestamptz) from public, anon, authenticated;
revoke all on function public.expire_qualified_reviewer_access() from public, anon, authenticated;
grant execute on function public.accept_qualified_reviewer_invite(text, text, timestamptz) to service_role;
grant execute on function public.expire_qualified_reviewer_access() to service_role;
