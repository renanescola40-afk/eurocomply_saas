...
create or replace function public.evaluate_enterprise_usage_alerts_v2_atomic(
...
begin
  ...
  for v_contract in
    ...
    where contract.contract_mode = 'negotiated'
      and contract.status in ('active','past_due','grace_period','read_only')
...
revoke all on function public.evaluate_enterprise_usage_alerts_atomic(integer) from service_role;
revoke all on function public.evaluate_enterprise_usage_alerts_v2_atomic(integer) from public, anon, authenticated;
grant execute on function public.evaluate_enterprise_usage_alerts_v2_atomic(integer) to service_role;
