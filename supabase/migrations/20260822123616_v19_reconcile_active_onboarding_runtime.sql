...
-- Forward-only production reconciliation for the active onboarding runtime.
...
begin
  if to_regclass('public.organizations') is null
     ...
     or to_regclass('public.onboarding_activation_runs') is null
     or to_regclass('public.ai_systems') is null
     or to_regclass('public.documents') is null
     or to_regclass('public.compliance_tasks') is null
     or to_regclass('public.invitations') is null then
    raise exception 'active onboarding runtime prerequisites are incomplete';
  end if;
  ...
  if not exists (
    ...
    where conrelid = 'public.invitations'::regclass
      and conname = 'invitations_organization_id_email_key'
      and contype = 'u'
  ) then
    raise exception 'tenant-scoped invitation uniqueness prerequisite is missing';
  end if;
  ...
  if not exists (
    ...
    raise exception 'controlled document storage reconciliation must precede onboarding activation';
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'compliance_tasks' and column_name = 'assigned_to'
  ) then
    raise exception 'compliance task assignee runtime prerequisite is missing';
  end if;
end
$preflight$;
...
alter table public.organizations
  ...
  drop constraint if exists organizations_readiness_score_check;

alter table public.organizations
  add constraint organizations_onboarding_status_check
    check (onboarding_status in ('not_started', 'in_progress', 'completed')),
  add constraint organizations_readiness_score_check
...
alter table public.onboarding_activation_runs
  add column if not exists ai_usage_summary text,
...
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'onboarding_activation_runs'
      and column_name = 'suggested_documents'
  ) then
    execute $sql$
      update public.onboarding_activation_runs
      set recommended_documents = coalesce(suggested_documents, '[]'::jsonb)
      where recommended_documents = '[]'::jsonb
    $sql$;
  end if;
  ...
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'onboarding_activation_runs'
      and column_name = 'created_tasks'
  ) then
    execute $sql$
      update public.onboarding_activation_runs
      set suggested_tasks = coalesce(created_tasks, '[]'::jsonb)
      where suggested_tasks = '[]'::jsonb
    $sql$;
  end if;
end
$legacy_backfill$;
...
do $verify$
declare
  activation_function_oid oid := to_regprocedure('public.complete_onboarding_activation_atomic(uuid,uuid,text,jsonb)');
  organization_column_count integer;
  activation_column_count integer;
begin
  select count(*)
  into organization_column_count
  ...
  if organization_column_count <> 11 then
    raise exception 'active onboarding organization columns are incomplete';
  end if;

  select count(*)
  into activation_column_count
  from information_schema.columns
  where table_schema = 'public'
  ...
  if activation_column_count <> 4 then
    raise exception 'active onboarding activation-run columns are incomplete';
  end if;
  ...
  if not exists (
    ...
    raise exception 'onboarding recommendation/task metadata columns are incomplete';
  end if;

  if activation_function_oid is null
     or has_function_privilege('anon', activation_function_oid, 'EXECUTE')
     or has_function_privilege('authenticated', activation_function_oid, 'EXECUTE')
     or not has_function_privilege('service_role', activation_function_oid, 'EXECUTE') then
    raise exception 'atomic onboarding activation function privileges are not canonical';
  end if;
  ...
  if not exists (
    ...
    where p.oid = activation_function_oid
      and p.prosecdef
      and setting = 'search_path=pg_catalog, public'
  ) then
    raise exception 'atomic onboarding activation security configuration is not fixed';
  end if;
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'onboarding_activation_runs'
      and indexname = 'onboarding_activation_runs_org_idempotency_key_idx'
      and indexdef ilike '%unique%'
  ) then
    raise exception 'atomic onboarding idempotency index is missing';
  end if;
end
$verify$;
