...
export function billingLifecycleRequestFingerprint(input: Pick<
  SubscriptionLifecycleInput,
  'action' | 'plan' | 'interval' | 'addOns'
>) {
...
function replaySnapshotFromResult(result: ReturnType<typeof lifecycleResult>): BillingLifecycleReplaySnapshot {
  return {
...
export async function mutateSubscriptionLifecycle(input: SubscriptionLifecycleInput) {
  ...
  await completeBillingLifecycleRequest(requestId, replaySnapshotFromResult(result));
  return result;
}
