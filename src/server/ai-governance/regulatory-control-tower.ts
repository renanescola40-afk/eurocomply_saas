...
const DEFINITIONS: Record<RegulatoryControlTowerWorkstreamId, { label:string; articleReference:string; weight:number; route:string|null }> = {
  ...
  prohibited_practices:{label:'Prohibited Practices',articleReference:'Article 5',weight:7,route:null},
  high_risk_provider_data:{label:'High-Risk Provider Data Governance',articleReference:'Article 10',weight:9,route:null},
...
export function buildRegulatoryControlTower(input:RegulatoryControlTowerInput):RegulatoryControlTowerDecision{const workstreams=REGULATORY_CONTROL_TOWER_WORKSTREAMS.map((id):RegulatoryControlTowerWorkstream=>{const definition=DEFINITIONS[id];const record=input[id]??null;const status=classify(record);return{id,label:definition.label,articleReference:definition.articleReference,weight:definition.weight,status,lifecycleState:record?.lifecycleState??null,recordId:record?.id??null,updatedAt:record?.updatedAt??null,route:definition.route,requiredAction:actionFor(status,definition)}});const totalWeight=workstreams.reduce((t,i)=>t+i.weight,0);const activatedWeight=workstreams.filter(i=>i.status!=='not_started').reduce((t,i)=>t+i.weight,0);const readyWeight=workstreams.filter(i=>i.status==='ready'||i.status==='not_applicable').reduce((t,i)=>t+i.weight,0);const blocked=workstreams.filter(i=>i.status==='blocked');const inProgress=workstreams.filter(i=>i.status==='in_progress');const notStarted=workstreams.filter(i=>i.status==='not_started');const ready=workstreams.filter(i=>i.status==='ready');const notApplicable=workstreams.filter(i=>i.status==='not_applicable');let overallStatus:RegulatoryControlTowerOverallStatus='not_started';if(blocked.length>0)overallStatus='blocked';else if(readyWeight===totalWeight)overallStatus='ready';else if(activatedWeight>0)overallStatus='in_progress';return{version:VERSION,overallStatus,activationPercent:Math.round(activatedWeight/totalWeight*100),readyPercent:Math.round(readyWeight/totalWeight*100),activatedWeight,readyWeight,totalWeight,readyCount:ready.length,blockedCount:blocked.length,inProgressCount:inProgress.length,notStartedCount:notStarted.length,notApplicableCount:notApplicable.length,workstreams,blockingWorkstreamIds:blocked.map(i=>i.id),requiredActions:workstreams.map(i=>i.requiredAction).filter((a):a is string=>Boolean(a)),evidenceBoundary:'This control tower aggregates persisted workflow lifecycle states for operational visibility. It does not validate underlying evidence, certify compliance, authorize market placement or replace legal, technical or conformity assessment review.'}}
