...
import { ADD_ON_CATALOG, getAddOnStatus, getPlanDisplayName } from './addons';
...
describe('add-on catalog adapter', () => {
  ...
  it('uses canonical customer-facing plan names for legacy aliases', () => {
    expect(getPlanDisplayName('starter')).toBe('Essential');
    expect(getPlanDisplayName('essential')).toBe('Essential');
    expect(getPlanDisplayName('growth')).toBe('Professional');
