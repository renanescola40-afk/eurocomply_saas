    ...
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white">
      ...
      <header className="relative z-10 border-b border-white/10 bg-[#050505]/78 px-6 py-4 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <Link href={`/${locale}`} className="flex items-center gap-3 rounded-md font-semibold tracking-tight text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 focus-visible:ring-offset-2 focus-visible:ring-offset-[#050505]">
            <span className="rounded-2xl border border-cyan-200/20 bg-cyan-300/[0.08] p-2 text-cyan-50">
              <ShieldCheck className="h-5 w-5" aria-hidden="true" />
            </span>
            RISCK COMPLY
          </Link>
          <nav className="hidden items-center gap-5 text-sm text-white/55 md:flex" aria-label={ui.portal}>
            <Link href={`/${locale}/trust`} className="rounded-md transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200">{ui.trust}</Link>
            <Link href={`/${locale}/security`} className="rounded-md transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200">{ui.security}</Link>
            <Link href={`/${locale}/privacy`} className="rounded-md transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200">{ui.privacy}</Link>
            <Link href={`/${locale}/status`} className="rounded-md transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200">{ui.status}</Link>
          </nav>
      ...
      <section className="relative z-10 border-b border-white/10 px-6 py-16 md:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.16fr_0.84fr] lg:items-start">
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-6 shadow-2xl backdrop-blur-xl">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-cyan-100/65">{ui.portal}</p>
            <div className="mt-5 grid gap-2">
              {pages.map((item) => (
                <Link key={item.slug} href={`/${locale}/${item.slug}`} className={`group flex items-center justify-between rounded-2xl border px-4 py-3 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 ${item.slug === page.slug ? 'border-white bg-white text-black' : 'border-white/10 bg-black/20 text-white/62 hover:border-cyan-200/25 hover:bg-cyan-300/[0.06] hover:text-white'}`}>
