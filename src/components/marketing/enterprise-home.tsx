...
const plans: Plan[] = [
  { name: 'Básico', price: '€49/mês', text: 'Para começar a organizar compliance sem caos.', features: ['Dashboard de compliance', 'Perfil fiscal europeu', 'Documentos controlados', 'Notícias regulatórias'] },
  { name: 'Pro', price: '€99/mês', text: 'Para equipas que já gerem risco operacional.', features: ['Calendário legal com IA', 'Matriz de riscos', 'Log de auditoria', 'Relatórios executivos'] },
  { name: 'Enterprise', price: '€249/mês', text: 'Para empresas multi-país que precisam de prova, velocidade e status.', highlighted: true, features: ['Múltiplos NIFs europeus', 'Convite de funcionários', 'Métricas avançadas preview', 'Suporte prioritário'] },
];
...
export function EnterpriseHome({ locale }: { locale: string }) {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-20"><div className="text-center"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-200">Planos</p><h2 className="mt-3 text-4xl font-black tracking-tight">Escolha o nível de tranquilidade da sua empresa.</h2></div><div className="mt-12 grid gap-6 lg:grid-cols-3">{plans.map((plan) => <div key={plan.name} className={`relative rounded-[2rem] border p-7 shadow-2xl transition hover:-translate-y-1 ${plan.highlighted ? 'border-amber-300/50 bg-amber-300/10' : 'border-white/10 bg-white/[0.04]'}`}>{plan.highlighted ? <div className="absolute -top-4 left-6 rounded-full bg-amber-300 px-4 py-2 text-xs font-black text-slate-950">⭐ Mais escolhido por empresas europeias</div> : null}<h3 className="text-2xl font-black">{plan.name}</h3><p className="mt-2 text-white/60">{plan.text}</p><p className="mt-6 text-4xl font-black">{plan.price}</p><ul className="mt-6 space-y-3 text-sm text-white/70">{plan.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><Link href={href(activeLocale, '/pricing')} className={`mt-8 block rounded-full px-5 py-4 text-center font-black transition hover:-translate-y-0.5 ${plan.highlighted ? 'bg-amber-300 text-slate-950 hover:bg-amber-200' : 'border border-white/15 text-white hover:bg-white/10'}`}>{plan.highlighted ? 'Desbloquear Enterprise' : 'Assinar agora'}</Link></div>)}</div></section>
