...
function getAuditReviewState(workflowReadiness: OrganizationWorkflowReadiness | undefined) {
  if (!workflowReadiness) return { label: 'Review inputs', description: 'Confirm readiness signals before starting audit package review.', tone: 'border-white/[0.075] bg-white/[0.025] text-white/52' };
  if (workflowReadiness.status === 'blocked') return { label: 'Blocked', description: 'Resolve blocking readiness signals before reviewing the audit package.', tone: 'border-rose-300/15 bg-rose-300/[0.055] text-rose-100/80' };
  if (workflowReadiness.status === 'attention') return { label: 'Needs attention', description: 'Review open readiness signals before confirming the audit package.', tone: 'border-amber-300/15 bg-amber-300/[0.055] text-amber-100/80' };
  return { label: 'Inputs healthy', description: 'Current workflow signals support a read-only audit package review from reports.', tone: 'border-emerald-300/15 bg-emerald-300/[0.055] text-emerald-100/80' };
}
...
export function AuditPackageReview({ summary, workflowReadiness, basePath = '/dashboard/organizations' }: AuditPackageReviewProps) {
  ...
  const metrics = [['Compliance score', `${summary.complianceScore}%`], ['Review signals', String(signalCount)], ['Open tasks', String(summary.openTasks)]];
      ...
      <div className="grid xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="px-5 py-5 md:px-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-200/65">Audit package review</p>
          ...
          <Link href={`${basePath}/reports`} className="mt-5 inline-flex h-10 items-center justify-center rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white transition hover:bg-blue-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/60">Review reports</Link>
        </div>
