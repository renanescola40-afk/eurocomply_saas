...
export function DashboardWorkspaceSidebar({ summary, basePath }: DashboardWorkspaceSidebarProps) {
  ...
  return (
    <aside className="premium-motion-enter sticky top-4 hidden max-h-[calc(100vh-2rem)] overflow-y-auto rounded-[2rem] border border-white/10 bg-slate-950 p-4 text-white shadow-2xl xl:block">
      <div className="premium-ambient-border premium-shimmer relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-4">
        ...
        <div className="relative">
          <div className="flex items-center gap-3">
            <div className="animate-pulse-border flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-sm font-black text-slate-950">EC</div>
            <div>
            ...
            <div className="mt-2 space-y-1.5">
              {group.items.map((item) => (
                <Link
                  ...
                  href={item.href}
                  className="premium-magnetic group flex items-center gap-3 rounded-2xl border border-transparent px-3 py-2.5 hover:border-white/10 hover:bg-white/[0.055]"
                >
                  <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border text-sm font-bold transition group-hover:scale-105 ${accentClasses(item.accent)}`}>{item.icon}</span>
                  ...
                  {item.signal !== undefined && (
                    <span className="rounded-full border border-white/10 bg-white/[0.045] px-2 py-1 text-[10px] font-bold text-slate-400 transition group-hover:border-primary/35 group-hover:text-slate-200">{item.signal}</span>
                  )}
                </Link>
              ))}
            </div>
