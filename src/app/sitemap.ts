...
const publicPaths = ['', '/trust', '/security', '/compliance', '/privacy', '/terms', '/dpa', '/subprocessors', '/status'] as const;
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return locales.flatMap((locale) =>
    publicPaths.map((path) => ({
      ...
      priority: path === '' ? 1 : path === '/trust' ? 0.85 : path === '/security' || path === '/compliance' ? 0.8 : path === '/status' ? 0.6 : 0.7,
      alternates: {
