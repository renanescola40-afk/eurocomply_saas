...
import { ArrowRight, Building2, CheckCircle2, Crown, Diamond, LockKeyhole, Mail, Plus, Save, Trash2, UploadCloud, UsersRound } from 'lucide-react';
...
export function ProfileClient({ locale }: { locale: string }) {
  ...
  function inviteEmployee() {
    ...
    setInviteEmail('');
    showToast('Convite simulado enviado e registado.');
  }
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-8 md:py-12">
        ...
        <form id="company-data" onSubmit={saveProfile} className="grid gap-6 lg:grid-cols-[1.12fr_0.88fr]">
          <section className="rounded-[2rem] border bg-background/88 p-6 shadow-sm backdrop-blur">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Button type="button" variant="outline" className="rounded-full" onClick={() => { updateCompany('logoUrl', 'https://dummyimage.com/180x180/d4af37/111827&text=EC'); showToast('Upload de logo simulado.'); }}><UploadCloud className="h-4 w-4" /> Simular upload de logo</Button>
          ...
          <aside id="enterprise-status" className="space-y-6">
            <section className={`rounded-[2rem] border p-6 shadow-sm backdrop-blur ${isEnterprise ? 'border-amber-300 bg-gradient-to-br from-amber-50 to-background shadow-amber-500/10 dark:from-amber-950/30' : 'bg-background/88'}`}>
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-sm text-muted-foreground">Plano atual</p><h2 className="mt-1 text-3xl font-semibold">{plan}</h2></div>
                <Badge className={`rounded-full ${isEnterprise ? 'bg-amber-500 text-black hover:bg-amber-500' : ''}`}>{isEnterprise ? <Diamond className="mr-1 h-3.5 w-3.5" /> : <Crown className="mr-1 h-3.5 w-3.5" />}{isEnterprise ? 'Enterprise Diamond' : 'Upgrade disponível'}</Badge>
              </div>
              <div className="mt-5 flex items-center gap-4">
                <div className={`relative h-20 w-20 overflow-hidden rounded-3xl border-4 bg-muted ${isEnterprise ? 'border-amber-400 shadow-lg shadow-amber-400/20' : 'border-border'}`}>
                  <Image src={company.logoUrl} alt="Avatar da empresa" fill sizes="80px" className="object-cover" unoptimized />
                </div>
                <div><p className="font-semibold">Avatar da empresa</p><p className="text-sm text-muted-foreground">{isEnterprise ? 'Borda premium Enterprise ativa.' : 'Borda premium disponível no Enterprise.'}</p></div>
              </div>
              <div className={`mt-5 rounded-2xl p-4 text-sm leading-6 ${isEnterprise ? 'border border-amber-300/70 bg-background/70' : 'bg-primary/10 text-primary'}`}>
                {isEnterprise ? '🏷️ Você está no plano mais alto — acesso prioritário em breve.' : '🔒 Desbloqueie múltiplos países fiscais com Enterprise e ganhe status visual, colaboração e prioridade.'}
                {isEnterprise ? <div className="mt-4 h-3 overflow-hidden rounded-full bg-amber-100"><div className="h-full w-2/3 animate-pulse rounded-full bg-amber-400" /></div> : null}
              </div>
              <Button asChild className="mt-5 w-full rounded-full transition hover:-translate-y-0.5 hover:shadow-lg"><Link href={`/${locale}/pricing`}>Comparar planos <ArrowRight className="h-4 w-4" /></Link></Button>
            </section>
          </aside>
        </form>
        ...
        <section className="rounded-[2rem] border bg-background/88 p-6 shadow-sm backdrop-blur">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div><h2 className="text-2xl font-semibold tracking-tight">Identificação fiscal por país</h2><p className="mt-1 text-sm text-muted-foreground">Portugal não é obrigatório. Cada país ajusta automaticamente o tipo de identificação, máscara e validação.</p></div>
          ...
          {!isEnterprise ? <div className="mt-5 rounded-2xl border border-dashed bg-muted/30 p-4 text-sm text-muted-foreground">🔒 No plano {plan}, a gestão multi-país aparece como preview bloqueado. Faça upgrade para Enterprise para usar múltiplas operações fiscais europeias.</div> : null}
              ...
              return (
                <div key={item.id} className={`relative rounded-2xl border p-4 transition ${!isEnterprise ? 'bg-muted/30 opacity-75' : 'bg-muted/15 hover:border-primary/40'}`}>
                  ...
                  <div className="grid gap-3 md:grid-cols-[0.85fr_1.15fr]">
                    <label><span className="text-sm font-medium">País</span><select disabled={!isEnterprise} value={item.country} onChange={(event) => updateFiscalId(item.id, 'country', event.target.value)} className="mt-1 w-full rounded-xl border bg-background px-3 py-2 text-sm outline-none focus:border-primary disabled:cursor-not-allowed">{(Object.keys(fiscalCountries) as CountryKey[]).map((country) => <option key={country}>{country}</option>)}</select></label>
                    <label><span className="text-sm font-medium">{meta.label}</span><input disabled={!isEnterprise} value={item.value} onChange={(event) => updateFiscalId(item.id, 'value', event.target.value)} placeholder={meta.placeholder} className={`mt-1 w-full rounded-xl border bg-background px-3 py-2 text-sm outline-none focus:border-primary disabled:cursor-not-allowed ${valid ? '' : 'border-red-500'}`} /><span className={`mt-1 block text-xs ${valid ? 'text-muted-foreground' : 'text-red-600'}`}>{valid ? meta.helper : `Formato esperado: ${meta.helper}`}</span></label>
                  </div>
        ...
        <section id="employees" className="rounded-[2rem] border bg-background/88 p-6 shadow-sm backdrop-blur">
          ...
          {isEnterprise ? (
            <div className="mt-5 space-y-5">
              <div className="grid gap-3 md:grid-cols-[1fr_220px_auto]"><input value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="email@empresa.com" className="rounded-2xl border bg-background px-4 py-3 text-sm outline-none focus:border-primary" /><select value={inviteRole} onChange={(event) => setInviteRole(event.target.value)} className="rounded-2xl border bg-background px-4 py-3 text-sm outline-none focus:border-primary"><option>Admin</option><option>Editor</option><option>Visualizador</option></select><Button type="button" onClick={inviteEmployee} className="rounded-full"><Mail className="h-4 w-4" /> Convidar funcionário</Button></div>
              <div className="grid gap-3 md:grid-cols-2">{employees.map((employee) => <div key={employee.id} className="rounded-2xl border bg-muted/20 p-4"><p className="font-semibold">{employee.name}</p><p className="text-sm text-muted-foreground">{employee.email}</p><div className="mt-3 flex gap-2"><Badge variant="outline">{employee.role}</Badge><Badge>{employee.status}</Badge></div></div>)}</div>
            </div>
          ) : (
            <div className="mt-5 rounded-2xl border bg-muted/30 p-5 text-sm leading-6 text-muted-foreground">🔒 Upgrade para o plano Enterprise e convide até 10 funcionários para colaborar na implementação dos documentos.<div><Button asChild className="mt-4 rounded-full"><Link href={`/${locale}/pricing`}>Fazer upgrade</Link></Button></div></div>
          )}
        </section>
      </div>
    </main>
