  ...
  switch (locale) {
    case 'pt':
      return {
        ...
        readyBody: 'Owners e Admins podem adicionar ou remover extensões elegíveis na subscrição existente. O acesso só muda depois de um evento Stripe assinado reconciliar o item no billing da organização.',
        releaseLocked: 'Os preços e a elegibilidade estão prontos, mas as alterações permanecem bloqueadas até o billing base, o exact-SHA de Produção e a autorização final do Owner serem aceitos.',
    ...
    case 'es':
      return {
        ...
        readyBody: 'Owners y Admins pueden añadir o eliminar extensiones elegibles de la suscripción existente. El acceso solo cambia después de que un evento firmado de Stripe reconcilie el elemento.',
        releaseLocked: 'Los precios y la elegibilidad están preparados, pero los cambios permanecen bloqueados hasta que el billing base, el exact-SHA de Producción y la autorización final del Owner hayan sido aceptados.',
    ...
    case 'fr':
      return {
        ...
        readyBody: 'Les Owners et Admins peuvent ajouter ou retirer des extensions éligibles de l’abonnement existant. L’accès ne change qu’après rapprochement d’un événement Stripe signé.',
        releaseLocked: 'Les prix et l’éligibilité sont prêts, mais les changements restent bloqués jusqu’à l’acceptation de la facturation de base, du SHA exact de Production et de l’autorisation finale de l’Owner.',
    ...
    case 'it':
      return {
        ...
        readyBody: 'Owner e Admin possono aggiungere o rimuovere estensioni idonee dall’abbonamento esistente. L’accesso cambia solo dopo la riconciliazione di un evento Stripe firmato.',
        releaseLocked: 'Prezzi e idoneità sono pronti, ma le modifiche restano bloccate finché billing base, exact-SHA di Produzione e autorizzazione finale dell’Owner non sono accettati.',
    ...
    case 'de':
      return {
        ...
        readyBody: 'Owner und Admins können berechtigte Erweiterungen zum bestehenden Abonnement hinzufügen oder daraus entfernen. Der Zugriff ändert sich erst nach dem Abgleich eines signierten Stripe-Ereignisses.',
        releaseLocked: 'Preise und Berechtigung sind vorbereitet, Änderungen bleiben jedoch gesperrt, bis Basis-Billing, der exakte Produktions-SHA und die endgültige Owner-Freigabe akzeptiert wurden.',
    ...
    default:
      return {
        ...
        readyBody: 'Owners and Admins can add or remove eligible extensions on the existing subscription. Access changes only after a signed Stripe event reconciles the item into organization billing.',
        releaseLocked: 'Pricing and eligibility are prepared, but changes remain locked until base billing, the exact Production SHA and final Owner authorization are accepted.',
                ...
                <article key={addOn.slug} id={`addon-${addOn.slug}`} className={`flex flex-col rounded-xl border bg-[#101715] p-5 ${isFocused ? 'border-emerald-300/25' : 'border-white/[0.075]'}`}>
                  ...
                  <div className="mt-auto pt-4">
                    <div className="flex items-end justify-between gap-4">
                      ...
                      {status === 'active' && canManageBilling && addOnCheckoutEnabled ? (
                        <BillingActionButton
                          action="replace_add_ons"
                          locale={locale}
                    ...
                    {status === 'active' && (!canManageBilling || !addOnCheckoutEnabled) ? <p className="mt-3 text-sm font-semibold text-emerald-200/80">{copy.active}</p> : null}
                    {status === 'preview' ? <p className="mt-3 text-xs leading-5 text-amber-100/65">{commerce.preview}</p> : null}
                    {(status === 'available' || status === 'active') && canManageBilling && !addOnCheckoutEnabled ? <p className="mt-3 text-xs leading-5 text-amber-100/65">{commerce.releaseLocked}</p> : null}
                    {(status === 'available' || status === 'active') && !canManageBilling ? <p className="mt-2 text-xs leading-5 text-amber-100/65">{copy.contactBillingAdmin}</p> : null}
