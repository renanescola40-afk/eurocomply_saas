...
const enterpriseDemoModules = [
  { title: 'Command Center', href: '/dashboard/organizations/command-center' },
  { title: 'AI Governance', href: '/ai-systems' },
  { title: 'AI Incidents', href: '/ai-incidents' },
  { title: 'Evidence & Risk', href: '/dashboard/organizations/evidence-risk' },
  { title: 'Reports & Governance', href: '/dashboard/organizations/reports-governance' },
  { title: 'Jornal IA Premium', href: '/dashboard/organizations/reports-governance/news' },
  { title: 'Enterprise Readiness', href: '/enterprise-readiness' },
  { title: 'Audit Pack', href: '/audit-pack' },
  { title: 'Evidence Verification', href: '/audit-pack/verify' },
  { title: 'Vendor Assurance', href: '/vendor-assurance' },
  { title: 'Compliance Calendar', href: '/calendario-compliance' },
  { title: 'Security Questionnaire', href: '/security-questionnaire' },
  { title: 'Retention Center', href: '/retention-center' },
  { title: 'Continuity Center', href: '/continuity-center' },
  { title: 'Riscos', href: '/riscos' },
...
type AddOnsAndCreditsPageProps = {
  params: Promise<{ locale: string }>;
  searchParams?: Promise<{ demo?: string }>;
};

export default async function AddOnsAndCreditsPage({ params, searchParams }: AddOnsAndCreditsPageProps) {
          ...
          <div className="grid gap-6 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-9">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-7 text-muted-foreground md:text-base">
                {isEnterpriseDemo
                  ? 'Esta simulação mostra o pacote Enterprise/Premium completo sem alterar Stripe, assinaturas reais ou permissões de produção.'
            ...
            <div className="rounded-[1.5rem] border bg-muted/30 p-5">
              ...
              <p className="mt-4 text-sm leading-6 text-muted-foreground">
                {isEnterpriseDemo
                  ? 'Modo de visualização comercial: todos os add-ons principais aparecem como incluídos no Premium.'
                  : isPremium
...
+              {enterpriseDemoModules.map((module) => (
+                <Link key={module.title} href={`/${locale}${module.href}`} className="rounded-2xl border bg-muted/20 p-4 text-sm font-semibold transition hover:border-primary/40 hover:bg-muted/40">
+                  {module.title}
+                </Link>
+              ))}
+            </div>
