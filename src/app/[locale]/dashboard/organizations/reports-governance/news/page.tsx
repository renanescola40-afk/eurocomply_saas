    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_hsl(var(--primary)/0.14),_transparent_34%),linear-gradient(180deg,_hsl(var(--background)),_hsl(var(--muted)/0.42))] text-foreground">
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-8 md:py-12">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          {[{ label: 'Matérias', value: intelligenceItems.length.toString(), icon: FileText }, { label: 'Impacto alto', value: intelligenceItems.filter((item) => item.impact === 'Alto' || item.impact === 'Crítico').length.toString(), icon: ShieldCheck }, { label: 'Calendário', value: intelligenceItems.length.toString(), icon: CalendarDays }, { label: 'Editorias', value: new Set(intelligenceItems.map((item) => item.persona.desk)).size.toString(), icon: Building2 }].map((metric) => {
        ...
        {wantsPremium && !canUsePremiumNews ? <UpgradeRequiredCard locale={locale} title="Notícias Premium ainda não estão ativas" description={`O plano básico mostra matérias essenciais. ${lockedCount} matérias premium ficam em preview até ativar Notícias Premium ou plano superior.`} ctaLabel="Ver add-ons" /> : null}
        ...
        <form className="grid gap-3 rounded-[1.5rem] border bg-background/90 p-5 shadow-sm md:grid-cols-4" action={`/${locale}/dashboard/organizations/reports-governance/news`}>
          <input name="q" defaultValue={query.q ?? ''} placeholder="Buscar tema, lei, país ou mercado" className="rounded-2xl border bg-background px-4 py-3 text-sm outline-none focus:border-primary" />
          <select name="jurisdiction" defaultValue={jurisdiction} className="rounded-2xl border bg-background px-4 py-3 text-sm outline-none focus:border-primary"><option value="all">Todas as regiões</option>{jurisdictions.map((item) => <option key={item} value={item}>{item}</option>)}</select>
          <select name="category" defaultValue={category} className="rounded-2xl border bg-background px-4 py-3 text-sm outline-none focus:border-primary"><option value="all">Todas as editorias</option>{categories.map((item) => <option key={item} value={item}>{item}</option>)}</select>
          <button className="rounded-2xl bg-foreground px-4 py-3 text-sm font-bold text-background transition hover:shadow-lg" type="submit">Filtrar jornal</button>
          ...
          {filtered.map((item) => {
            ...
            return (
              <article key={item.id} className="rounded-[2rem] border bg-background/92 p-6 shadow-sm md:p-8">
                ...
                <div className="mt-5 grid gap-3 text-sm text-muted-foreground md:grid-cols-4"><p className="flex items-center gap-2"><UserRound className="h-4 w-4" /> {item.persona.name}</p><p className="flex items-center gap-2"><FileText className="h-4 w-4" /> {item.source}</p><p className="flex items-center gap-2"><CalendarDays className="h-4 w-4" /> {formatDate(item.publishedAt, locale)}</p><p className="flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> {item.reliability}</p></div>
                {locked ? <div className="mt-6 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-800 dark:text-amber-100">Matéria premium em preview. Ative Notícias Premium para ver análise completa e ações detalhadas.</div> : <div className="mt-6 grid gap-4 lg:grid-cols-2"><section className="rounded-[1.5rem] border bg-muted/20 p-5"><h3 className="text-sm font-bold uppercase tracking-[0.18em] text-muted-foreground">Matéria completa</h3><p className="mt-3 text-sm leading-7">{item.articleParagraphs[0]}</p><Link href={detailHref} className="mt-4 inline-flex rounded-full bg-foreground px-4 py-2 text-xs font-bold text-background transition hover:opacity-90">Ler jornal completo</Link></section><section className="rounded-[1.5rem] border bg-muted/20 p-5"><h3 className="text-sm font-bold uppercase tracking-[0.18em] text-muted-foreground">Calendário inteligente</h3><p className="mt-3 text-sm leading-6">{item.calendarSuggestion}</p><Link href={buildCalendarSuggestionHref(locale, item)} className="mt-4 inline-flex rounded-full border px-4 py-2 text-xs font-bold transition hover:bg-background">Adicionar ao calendário</Link></section></div>}
        ...
        {filtered.length === 0 ? <p className="rounded-[1.5rem] border border-dashed bg-background/80 p-8 text-center text-sm font-semibold text-muted-foreground">Nenhuma matéria encontrada com estes filtros.</p> : null}
      </div>
    </main>
  );
}
