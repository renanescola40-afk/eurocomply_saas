...
export default async function PrintableExecutiveReportPage({ params }: { params: { locale: string } }) {
  ...
  return (
    <main className="mx-auto max-w-4xl bg-white px-8 py-10 text-slate-950 print:px-0 print:py-0">
      <div className="mb-8 flex items-center justify-between gap-4 print:hidden">
        <Link href={`/${params.locale}/dashboard/organizations/reports`} className="rounded-md border px-4 py-2 text-sm font-medium hover:bg-slate-50">
          Back to reports
        </Link>
        <button type="button" onClick={() => undefined} className="rounded-md border px-4 py-2 text-sm font-medium text-slate-400" disabled>
          ...
          <tbody>
            {metrics.map(([label, value]) => (
              <tr key={label} className="border-b">
                <th className="py-3 font-medium text-slate-600">{label}</th>
      ...
      <footer className="mt-8 border-t pt-6 text-xs text-slate-500">
