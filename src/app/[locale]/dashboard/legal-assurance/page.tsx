...
export default function LegalAssurancePage() {
  ...
  const activeCount = useMemo(
    () => reviews.filter((review) => !['COMPLETED', 'DECLINED', 'CANCELLED', 'EXPIRED', 'SUPERSEDED'].includes(review.status)).length,
    [reviews],
  );
  ...
  return (
    <main className="space-y-6 text-white">
      <Button variant="ghost" onClick={() => router.push(`/${locale}/dashboard`)} className="h-9 px-2 text-white/50 hover:bg-white/[0.05] hover:text-white"><ArrowLeft className="mr-2 h-4 w-4" />{t.back}</Button>
      <header className="border-b border-white/[0.07] pb-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0"><Badge className="mb-3 rounded-lg border-violet-300/15 bg-violet-300/[0.08] text-violet-200"><Scale className="mr-1.5 h-3.5 w-3.5" />{t.badge}</Badge><h1 className="max-w-4xl text-3xl font-semibold tracking-tight md:text-4xl">{t.title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-white/48 md:text-base">{t.subtitle}</p><p className="mt-2 text-xs text-amber-200/70">{t.pending}</p></div>
          <Button onClick={() => void loadReviews()} variant="outline" className="border-white/[0.09] bg-white/[0.025] text-white/70 hover:bg-white/[0.06] hover:text-white"><RefreshCw className="mr-2 h-4 w-4" />{t.refresh}</Button>
        </div>
      </header>

      {state === 'unavailable' ? (
        <Card className="rounded-xl border-white/[0.075] bg-[#0d1522] text-white"><CardContent className="py-14 text-center"><ShieldCheck className="mx-auto h-8 w-8 text-white/30" /><p className="mt-4 text-sm text-white/50">{t.unavailable}</p></CardContent></Card>
      ) : (
        <>
          <section className="grid gap-3 md:grid-cols-3"><Signal label={t.current} value={state === 'loading' ? '--' : String(reviews.length)} icon={<BriefcaseBusiness className="h-4 w-4" />} /><Signal label="Active" value={state === 'loading' ? '--' : String(activeCount)} icon={<FileCheck2 className="h-4 w-4" />} /><Signal label="External Validation" value="PENDING" icon={<BadgeCheck className="h-4 w-4" />} /></section>
          <Card className="rounded-xl border-white/[0.075] bg-[#0d1522] text-white"><CardHeader><CardTitle className="text-base">{t.request}</CardTitle><CardDescription className="text-white/40">RISCK COMPLY prepares the operational record; independent Counsel retains professional judgment.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-[1fr_1fr_160px_auto]"><Input aria-label={t.type} value={reviewType} onChange={(event) => setReviewType(event.target.value)} placeholder={t.type} className="border-white/[0.09] bg-white/[0.03] text-white" /><Input aria-label={t.jurisdiction} value={jurisdiction} onChange={(event) => setJurisdiction(event.target.value)} placeholder={t.jurisdiction} className="border-white/[0.09] bg-white/[0.03] text-white" /><select aria-label={t.priority} value={priority} onChange={(event) => setPriority(event.target.value)} className="h-10 rounded-md border border-white/[0.09] bg-[#0a111c] px-3 text-sm text-white/80"><option value="LOW">LOW</option><option value="NORMAL">NORMAL</option><option value="HIGH">HIGH</option><option value="URGENT">URGENT</option></select><Button onClick={() => void requestReview()} disabled={submitting || state !== 'ready'} className="bg-violet-600 text-white hover:bg-violet-500"><Plus className="mr-2 h-4 w-4" />{submitting ? '...' : t.request}</Button></CardContent></Card>
          ...
          <Card className="overflow-hidden rounded-xl border-white/[0.075] bg-[#0d1522] text-white"><CardHeader className="border-b border-white/[0.07]"><CardTitle className="text-base">{t.current}</CardTitle><CardDescription className="text-white/40">{reviews.length} total</CardDescription></CardHeader><CardContent className="divide-y divide-white/[0.06] p-0">{state === 'loading' ? <p className="py-12 text-center text-sm text-white/40">...</p> : reviews.length === 0 ? <p className="py-12 text-center text-sm text-white/40">{t.empty}</p> : reviews.map((review) => (
            <article key={review.id} className="grid gap-4 p-5 lg:grid-cols-[1.25fr_.7fr_.7fr_.7fr_auto] lg:items-center">
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-medium text-white/90">{review.review_type}</h2><Badge className={statusTone(review.status)}>{review.status}</Badge></div><p className="mt-2 text-xs text-white/38">{t.requested}: {new Date(review.requested_at).toLocaleString(locale)} · {review.jurisdiction} · {review.priority}</p></div>
              <Meta label={t.counsel} value={review.assigned_counsel_id ?? '—'} /><Meta label={t.firm} value={review.law_firm_id ?? '—'} /><Meta label={t.expiry} value={review.expires_at ? new Date(review.expires_at).toLocaleDateString(locale) : '—'} />
              <Button size="sm" variant="outline" onClick={() => router.push(`/${locale}/dashboard/legal-assurance/${review.id}`)} className="border-violet-300/20 bg-violet-300/[0.04] text-violet-100 hover:bg-violet-300/[0.08]">{t.open}</Button>
            </article>
          ))}</CardContent></Card>
        </>
      )}
