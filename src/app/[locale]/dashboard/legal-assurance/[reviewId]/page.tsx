...
import { ArrowLeft, Download, FileCheck2, RefreshCw, Send, ShieldAlert } from 'lucide-react';
...
export default function LegalAssuranceMatterPage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'legal_review_load_failed'); }
  ...
  async function action(body: Record<string, unknown>) {
    ...
    try {
      const response = await fetch(`/api/legal-assurance/${encodeURIComponent(review.id)}`, { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...body, expectedUpdatedAt: review.updated_at }) });
      if (!response.ok) { const failure = await response.json().catch(() => ({})) as { error?: string }; throw new Error(failure.error ?? 'legal_review_operation_failed'); }
      await load();
    } catch (actionError) { setError(actionError instanceof Error ? actionError.message : 'legal_review_operation_failed'); }
  ...
  return <main className="space-y-6 text-white">
    ...
    {canResubmit && (matter?.remediation.length ?? 0) > 0 ? <Card className="rounded-xl border-white/[0.075] bg-[#0d1522] text-white"><CardHeader><CardTitle className="text-base">{t.remediation}</CardTitle><CardDescription className="text-white/40">Counsel findings are addressed individually; backend state remains authoritative.</CardDescription></CardHeader><CardContent className="space-y-4">{matter?.remediation.map((row) => { const id = String(row.id ?? ''); return <div key={id} className="space-y-3 rounded-lg border border-white/[0.06] bg-white/[0.02] p-4"><div className="grid gap-2 md:grid-cols-3"><Meta label="Finding" value={text(row.stable_finding_id)} /><Meta label={t.status} value={text(row.status)} /><Meta label="Severity" value={text(row.severity)} /></div><p className="text-sm font-medium text-white/80">{text(row.title)}</p><p className="text-sm text-white/52">{text(row.required_action)}</p><textarea aria-label={`${t.response}-${id}`} value={remediationAnswers[id] ?? ''} onChange={(event) => setRemediationAnswers((current) => ({ ...current, [id]: event.target.value }))} rows={4} className="w-full rounded-md border border-white/[0.09] bg-[#0a111c] p-3 text-sm text-white outline-none" /><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => void action({ action: 'UPDATE_REMEDIATION', remediationId: id, customerResponse: { response: (remediationAnswers[id] ?? '').trim() }, markReady: false })} disabled={busy || !(remediationAnswers[id]?.trim())} className="border-white/[0.09] bg-white/[0.025] text-white/70">{t.save}</Button><Button size="sm" onClick={() => void action({ action: 'UPDATE_REMEDIATION', remediationId: id, customerResponse: { response: (remediationAnswers[id] ?? '').trim() }, markReady: true })} disabled={busy || !(remediationAnswers[id]?.trim())} className="bg-emerald-600 hover:bg-emerald-500">{t.ready}</Button></div></div>; })}</CardContent></Card> : null}

    {canResubmit ? <Card className="rounded-xl border-amber-300/15 bg-amber-300/[0.04] text-white"><CardHeader><CardTitle className="text-base">{t.resubmit}</CardTitle><CardDescription className="text-white/40">Resubmission succeeds only when the backend verifies required remediation is ready.</CardDescription></CardHeader><CardContent><Button onClick={() => void action({ action: 'RESUBMIT' })} disabled={busy} className="bg-amber-600 hover:bg-amber-500">{t.resubmit}</Button></CardContent></Card> : null}

    <section className="grid gap-4 xl:grid-cols-2">
      <MatterCard title={t.packages} empty={t.empty} rows={matter?.packages ?? []} render={(row) => <><Meta label={t.version} value={String(row.package_version ?? '—')} /><Meta label={t.release} value={text(row.product_release_sha)} mono /><Meta label={t.manifest} value={text(row.package_manifest_digest)} mono /></>} />
      <MatterCard title={t.decisions} empty={t.empty} rows={matter?.decisions ?? []} render={(row) => <><Meta label={t.decision} value={text(row.decision)} /><Meta label={t.jurisdiction} value={text(row.jurisdiction)} /><Meta label={t.digest} value={text(row.decision_digest)} mono /></>} />
