...
const copy = {
  en: { back: 'Back to dashboard', badge: 'Board Report', title: 'Executive AI compliance report', subtitle: 'Board-ready summary of risk, readiness and recommended actions.', generate: 'Refresh report', txt: 'Export TXT', pdf: 'Export PDF', status: 'Board Status', score: 'Compliance Score', readiness: 'Audit Readiness', evidence: 'Evidence Coverage', findings: 'Critical Findings', tasks: 'Open Tasks', summary: 'Executive Summary', risks: 'Key Risks', actions: 'Recommended Next Actions', empty: 'No board data yet. Run Gap Analysis and add evidence first.' },
  pt: { back: 'Voltar ao dashboard', badge: 'Board Report', title: 'Relatório executivo de compliance de IA', subtitle: 'Resumo para diretoria sobre risco, prontidão e ações recomendadas.', generate: 'Atualizar relatório', txt: 'Exportar TXT', pdf: 'Exportar PDF', status: 'Status para Diretoria', score: 'Score de Compliance', readiness: 'Prontidão de Auditoria', evidence: 'Cobertura de Evidências', findings: 'Findings Críticos', tasks: 'Tarefas Abertas', summary: 'Resumo Executivo', risks: 'Riscos Principais', actions: 'Próximas Ações Recomendadas', empty: 'Ainda não há dados executivos. Execute o Gap Analysis e adicione evidências primeiro.' },
  es: { back: 'Volver al dashboard', badge: 'Board Report', title: 'Informe ejecutivo de compliance de IA', subtitle: 'Resumen para dirección sobre riesgo, preparación y acciones.', generate: 'Actualizar informe', txt: 'Exportar TXT', pdf: 'Exportar PDF', status: 'Estado para dirección', score: 'Puntuación de compliance', readiness: 'Preparación de auditoría', evidence: 'Cobertura de evidencias', findings: 'Hallazgos críticos', tasks: 'Tareas abiertas', summary: 'Resumen ejecutivo', risks: 'Riesgos principales', actions: 'Acciones recomendadas', empty: 'Aún no hay datos. Ejecuta Gap Analysis y agrega evidencias primero.' },
  fr: { back: 'Retour au dashboard', badge: 'Board Report', title: 'Rapport exécutif conformité IA', subtitle: 'Synthèse pour comité : risque, préparation et actions.', generate: 'Actualiser le rapport', txt: 'Exporter TXT', pdf: 'Exporter PDF', status: 'Statut comité', score: 'Score conformité', readiness: 'Préparation audit', evidence: 'Couverture preuves', findings: 'Écarts critiques', tasks: 'Tâches ouvertes', summary: 'Résumé exécutif', risks: 'Risques clés', actions: 'Actions recommandées', empty: 'Aucune donnée. Lancez Gap Analysis et ajoutez des preuves d’abord.' },
  it: { back: 'Torna alla dashboard', badge: 'Board Report', title: 'Report executive compliance IA', subtitle: 'Sintesi board su rischio, readiness e azioni.', generate: 'Aggiorna report', txt: 'Esporta TXT', pdf: 'Esporta PDF', status: 'Stato Board', score: 'Punteggio compliance', readiness: 'Audit Readiness', evidence: 'Copertura evidenze', findings: 'Finding critici', tasks: 'Task aperti', summary: 'Executive Summary', risks: 'Rischi principali', actions: 'Azioni raccomandate', empty: 'Nessun dato. Esegui Gap Analysis e aggiungi evidenze prima.' },
  de: { back: 'Zurück zum Dashboard', badge: 'Board Report', title: 'Executive AI Compliance Report', subtitle: 'Board-Zusammenfassung zu Risiko, Readiness und Maßnahmen.', generate: 'Report aktualisieren', txt: 'TXT exportieren', pdf: 'PDF exportieren', status: 'Board Status', score: 'Compliance Score', readiness: 'Audit Readiness', evidence: 'Nachweisabdeckung', findings: 'Kritische Findings', tasks: 'Offene Aufgaben', summary: 'Executive Summary', risks: 'Wesentliche Risiken', actions: 'Empfohlene Maßnahmen', empty: 'Noch keine Daten. Starten Sie Gap Analysis und fügen Sie Nachweise hinzu.' },
} as const;
...
export default function BoardReportPage() {
  ...
  function exportTxt() {
    if (!data) return;
    downloadTextReport({ filename: 'eurocomply-board-report.txt', content: boardReportToText(data) });
  }
...
function ListCard({ title, items }: { title: string; items: string[] }) {
  return <Card data-print-card className="border-white/10 bg-white/[0.045] text-white"><CardHeader><CardTitle>{title}</CardTitle><CardDescription className="text-white/48">Board-level summary</CardDescription></CardHeader><CardContent className="space-y-3">{items.map((item, index) => <div key={`${item}-${index}`} className="rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white/70">{item}</div>)}</CardContent></Card>;
}
