...
const copy: Record<Locale, { eyebrow: string; title: string; summary: string; sections: Array<{ title: string; paragraphs?: string[]; items?: string[] }> }> = {
  en: {
    ...
    sections: [
      ...
      { title: 'Provider and retention verification', paragraphs: ['Cookie names, storage duration, analytics region, provider retention and transfer treatment must be reconciled with the live production configuration and a documented legal basis before final publication; qualified review is escalated only where a specific law, contract or buyer requirement makes it necessary.'] },
    ],
  },
  pt: {
    ...
    sections: [
      ...
      { title: 'Verificação de fornecedor e conservação', paragraphs: ['Nomes de cookies, duração, região de analytics, conservação do fornecedor e tratamento de transferências devem ser reconciliados com a configuração LIVE e uma base jurídica documentada antes da publicação final; a revisão qualificada só é escalada quando exigida por lei, contrato ou comprador.'] },
    ],
  },
  es: {
