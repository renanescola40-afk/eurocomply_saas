...
function LoginContent() {
  ...
  const [error, setError] = useState<string | null>(authErrorCode ? errorMessages[authErrorCode] : null);
  ...
  async function handleProvider() {
    ...
    if (result.error) {
      setError(errorMessages.auth_exchange_failed);
      setBusy(false);
    }
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.error) {
      setError(errorMessages.email_sign_in_failed);
      setBusy(false);
      return;
    }
