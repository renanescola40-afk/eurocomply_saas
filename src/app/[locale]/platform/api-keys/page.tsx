...
import { EnterpriseApiKey } from '@/components/platform/enterprise-api-key';
...
export default async function EnterpriseApiKeysPage({ params }: PageProps) {
  noStore();
  const { locale } = await params;
  const resolvedLocale = safeLocale(locale);
  const user = await getCurrentUser();
  if (!user) redirect(`/${resolvedLocale}/login?next=/${resolvedLocale}/platform/api-keys`);
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(37,99,235,0.18),_transparent_34rem),linear-gradient(180deg,#050505_0%,#080b12_48%,#050505_100%)] text-white">
      <div className="mx-auto max-w-5xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <header>
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight md:text-5xl">Enterprise API credentials</h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-white/60">
            Issue organization-bound service credentials. Plaintext is never stored and is displayed only in the creation response.
          </p>
        </header>
        <EnterpriseApiKey />
      </div>
