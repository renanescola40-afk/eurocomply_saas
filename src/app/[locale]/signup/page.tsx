...
function getSignupCopy(activeLocale: Locale) {
  return activeLocale === 'pt'
    ? {
        ...
        verifySubtitle: 'Enviámos um link para confirmar a conta. Depois entre para continuar o onboarding.',
    ...
    : {
        ...
        verifySubtitle: 'We sent a confirmation link. Then sign in to continue onboarding.',
...
function SignupAuthForm({ activeLocale, selectedPlan, continuationHref, signInUrl }: SignupAuthFormProps) {
  ...
  return (
    <SignupChrome activeLocale={activeLocale}>
      <SignupHeader title={pendingVerification ? text.verifyTitle : text.title} subtitle={pendingVerification ? text.verifySubtitle : text.subtitle} />
      ...
      {pendingVerification ? (
        <Link href={signInUrl} className="mt-6 flex w-full items-center justify-center rounded-2xl bg-blue-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-400">
          {text.signIn}
        </Link>
      ) : (
        <>
          ...
          >
            {isSubmitting ? text.loading : text.google}
          </button>
          ...
          <form onSubmit={handleEmailSignUp} className="space-y-4">
            ...
            <label className="block text-sm font-medium text-white/70">
              {text.password}
              <input type="password" value={secret} onChange={(event) => setSecret(event.target.value)} required minLength={8} autoComplete="new-password" className="mt-2 w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-white outline-none transition placeholder:text-white/30 focus:border-blue-300/60" />
            </label>
            <button type="submit" disabled={isSubmitting} className="w-full rounded-2xl bg-blue-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-60">
