...
export default async function SalesLeadDetailPage({ params, searchParams }: PageProps) {
  ...
  const [lead, notes, activities] = await Promise.all([
    ...
    listSalesLeadActivityEvents(id),
  ]);
        ...
        <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-2xl">
              <h2 className="text-xl font-semibold">Qualification snapshot</h2>
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/40">Message</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/75">{lead.message ?? '—'}</p></div>
            </div>
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-2xl">
              ...
              <div className="mt-5 space-y-3">
                {activities.map((activity) => <article key={activity.id} className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-sm font-semibold text-white/85">{activity.action}</p><p className="mt-2 text-xs text-white/35">{formatDate(activity.created_at)}</p></article>)}
