...
import { listSalesLeads, normalizeSalesLeadFilters, SALES_LEAD_STATUSES } from '@/server/queries/sales-leads';
...
export default async function SalesLeadsPage({ params, searchParams }: PageProps) {
  ...
  const result = await listSalesLeads(filters);
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(37,99,235,0.18),_transparent_34rem),linear-gradient(180deg,#050505_0%,#080b12_48%,#050505_100%)] text-white">
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/60">
              Internal Early Access pipeline for demo requests. This is not a customer-facing CRM and must not be exposed in tenant workspaces.
            </p>
          </div>
        ...
        <form className="grid gap-3 rounded-[2rem] border border-white/10 bg-white/[0.04] p-4 shadow-2xl md:grid-cols-6" action={`/${safeLocale}/admin/sales/leads`}>
          ...
          <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/50">
            Source
            <input name="source" defaultValue={filters.source ?? ''} placeholder="book-demo-page" className="h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-blue-200/40" />
          </label>
          <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/50">
            Timeline
            <input name="timeline" defaultValue={filters.timeline ?? ''} placeholder="1-3 months" className="h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-blue-200/40" />
          </label>
          <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/50">
            Company size
            <input name="companySize" defaultValue={filters.companySize ?? ''} placeholder="51-200" className="h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-blue-200/40" />
          </label>
          <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/50">
            From
            <input name="from" type="date" defaultValue={Array.isArray(rawSearchParams.from) ? rawSearchParams.from[0] : rawSearchParams.from ?? ''} className="h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-4 text-sm text-white outline-none focus:border-blue-200/40" />
          </label>
          <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/50">
            To
            <input name="to" type="date" defaultValue={Array.isArray(rawSearchParams.to) ? rawSearchParams.to[0] : rawSearchParams.to ?? ''} className="h-11 w-full rounded-2xl border border-white/10 bg-black/35 px-4 text-sm text-white outline-none focus:border-blue-200/40" />
          </label>
          <div className="flex items-end gap-3 md:col-span-4">
            <button className="h-11 rounded-full bg-white px-5 text-sm font-semibold text-black transition hover:bg-zinc-200" type="submit">Apply filters</button>
            <Link href={`/${safeLocale}/admin/sales/leads`} className="inline-flex h-11 items-center rounded-full border border-white/10 px-5 text-sm font-semibold text-white/70 transition hover:border-white/25 hover:text-white">Clear</Link>
          </div>
        </form>
            ...
            <table className="min-w-full divide-y divide-white/10 text-sm">
              ...
              <tbody className="divide-y divide-white/10 text-white/75">
                {result.leads.map((lead) => (
                  <tr key={lead.id} className="transition hover:bg-white/[0.04]">
                    <td className="px-5 py-4">
                      <Link href={`/${safeLocale}/admin/sales/leads/${lead.id}`} className="font-semibold text-white hover:text-blue-100">{lead.full_name}</Link>
                      <p className="mt-1 text-xs text-white/45">{lead.work_email}</p>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-medium text-white/90">{lead.company_name}</span>
                ...
                {result.leads.length === 0 ? (
                  <tr><td className="px-5 py-12 text-center text-white/50" colSpan={7}>No leads match these filters.</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
