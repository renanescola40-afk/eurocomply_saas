...
import { ArrowLeft, FileCheck2, RefreshCw, Scale, Send, ShieldAlert } from 'lucide-react';
...
export default function CounselReviewCockpitPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 403 || response.status === 404) {
        setPayload(null);
        setError('counsel_review_unavailable');
        return;
      }
  ...
  async function action(body: Record<string, unknown>) {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/counsel/legal-reviews/${encodeURIComponent(review.id)}`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(failure.error ?? 'counsel_review_operation_failed');
      }
      await load();
    } catch (actionError) {
      setError(actionError instanceof Error ? actionError.message : 'counsel_review_operation_failed');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function issueDecision() {
    ...
    const remediationItems = decision === 'REMEDIATION_REQUIRED'
      ? [{ stableFindingId: findingId.trim(), title: findingTitle.trim(), requiredAction: requiredAction.trim(), severity }]
      : [];
    ...
    <main className="space-y-6 text-white">
      ...
      <header className="border-b border-white/[0.07] pb-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div><Badge className="mb-3 border-violet-300/15 bg-violet-300/[0.08] text-violet-200"><Scale className="mr-1.5 h-3.5 w-3.5" />{t.badge}</Badge><h1 className="text-3xl font-semibold tracking-tight md:text-4xl">{t.title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-white/48">{t.subtitle}</p><p className="mt-2 text-xs text-amber-200/70">{t.pending}</p></div>
          <Button onClick={() => void load()} variant="outline" disabled={loading || busy} className="border-white/[0.09] bg-white/[0.025] text-white/70"><RefreshCw className="mr-2 h-4 w-4" />{t.refresh}</Button>
        </div>
      </header>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <Signal label={t.status} value={review?.status ?? (loading ? '...' : '—')} />
        <Signal label={t.packageVersion} value={latestPackage ? String(latestPackage.package_version ?? '—') : '—'} />
        <Signal label={t.updated} value={review ? new Date(review.updated_at).toLocaleString(locale) : '—'} />
      </section>

      {canStart ? <Card className="rounded-xl border-violet-300/15 bg-violet-300/[0.04] text-white"><CardHeader><CardTitle className="text-base">{t.start}</CardTitle><CardDescription className="text-white/45">The matter is package-ready and can enter professional review.</CardDescription></CardHeader><CardContent><Button onClick={() => void action({ action: 'START_REVIEW' })} disabled={busy} className="bg-violet-600 hover:bg-violet-500"><FileCheck2 className="mr-2 h-4 w-4" />{t.start}</Button></CardContent></Card> : null}
