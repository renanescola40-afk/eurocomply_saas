...
export default async function CheckoutPage({ params, searchParams }: CheckoutPageProps) {
  ...
  const nextPath = `/${locale}/checkout?plan=${selectedPlan.id}`;
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            {user ? (
              ...
              <Link href={`/${locale}/login?next=${encodeURIComponent(nextPath)}`} className="rounded-full bg-white px-4 py-2 font-semibold text-black hover:bg-white/90">Sign in</Link>
            )}
          </nav>
        </div>
      </header>
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-slate-950 p-6 shadow-2xl">
            ...
            {organization ? (
              ...
              <div className="mt-6 grid gap-3">
                <Link href={`/${locale}/signup?plan=${selectedPlan.id}&next=${encodeURIComponent(nextPath)}`} className="flex h-12 w-full items-center justify-center rounded-full bg-white px-6 text-sm font-bold text-black hover:bg-white/90">
                  Create account and continue
                </Link>
                <Link href={`/${locale}/login?next=${encodeURIComponent(nextPath)}`} className="flex h-12 w-full items-center justify-center rounded-full border border-white/15 px-6 text-sm font-bold hover:bg-white/10">
                  Sign in to continue
                </Link>
              </div>
            )}
