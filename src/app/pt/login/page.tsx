...
export default function PortugueseLoginPage() {
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md items-center px-5">
        <div className="w-full rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-7 shadow-2xl backdrop-blur-xl">
          <Link href="/pt" className="mb-8 flex items-center gap-3">
          ...
          <Button type="button" variant="outline" className="mb-4 w-full border-white/10 bg-white/[0.04] text-white hover:bg-white/[0.08] hover:text-white" onClick={handleGoogleLogin} disabled={loading || authLoading}>
            Continuar com Google
          </Button>

          <form onSubmit={handleEmailLogin} className="space-y-4">
            <Input className="border-white/10 bg-white/[0.05] text-white placeholder:text-white/35" type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required />

            <div className="relative">
              <Input className="border-white/10 bg-white/[0.05] pr-10 text-white placeholder:text-white/35" type={showPassword ? 'text' : 'password'} placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} required />
            ...
            <Button type="submit" className="w-full bg-white text-black hover:bg-white/90" disabled={loading || authLoading}>
          ...
          <p className="mt-5 text-center text-sm text-white/52">
            Ainda não tem conta?{' '}
            <Link href="/pt" className="text-white hover:underline">Criar conta</Link>
          </p>
        </div>
      </div>
    </main>
