...
import { acceptQmsAudit, approveQmsManagementReview, approveQmsSystem, closeQmsNonconformity, completeQmsControl, configureQmsSystem, createQmsAudit, createQmsControl, createQmsManagementReview, createQmsNonconformity, createQmsSystem, listQmsSnapshot, rollbackQmsApproval, rollbackQmsCreate } from '@/server/queries/qms-governance';
...
const workflows = ['system_create','system_configure','control_create','control_complete','nonconformity_create','nonconformity_close','audit_create','audit_accept','management_review_create','management_review_approve','system_approve'] as const;
type Workflow = (typeof workflows)[number];
const sha256 = z.string().regex(/^[a-f0-9]{64}$/);
const expected = z.string().datetime({ offset: true });
...
const configureSchema = z.object({ qmsSystemId:z.string().uuid(), expectedUpdatedAt:expected, reviewerUserId:z.string().uuid(), approverUserId:z.string().uuid() });
...
const controlCompleteSchema = z.object({ controlId:z.string().uuid(), expectedUpdatedAt:expected, status:z.enum(['effective','not_applicable']), rationale:z.string().trim().min(10).max(4000), evidenceReference:z.string().trim().min(3).max(1000).nullable(), evidenceDigest:sha256.nullable() });
...
const ncCloseSchema = z.object({ nonconformityId:z.string().uuid(), expectedUpdatedAt:expected, rootCause:z.string().trim().min(10).max(4000), correctiveAction:z.string().trim().min(10).max(4000) });
...
const auditAcceptSchema = z.object({ auditId:z.string().uuid(), expectedUpdatedAt:expected, findingsCount:z.number().int().min(0), highFindingsCount:z.number().int().min(0), criticalFindingsCount:z.number().int().min(0), reportReference:z.string().trim().min(3).max(1000), reportDigest:sha256 });
const reviewSchema = z.object({ qmsSystemId:z.string().uuid(), periodStart:z.string().date(), periodEnd:z.string().date() });
const reviewApproveSchema = z.object({ reviewId:z.string().uuid(), expectedUpdatedAt:expected, reviewerUserId:z.string().uuid(), inputsSummary:z.string().trim().min(20).max(8000), decisionsSummary:z.string().trim().min(20).max(8000), evidenceReference:z.string().trim().min(3).max(1000), evidenceDigest:sha256 });
const approveSchema = z.object({ qmsSystemId:z.string().uuid(), expectedUpdatedAt:expected, rationale:z.string().trim().min(10).max(4000) });
...
export async function POST(request:Request){
  try {
    ...
    const permission=await assertOrganizationPermission({userId:user.id,organizationId:organization.id,permission:'manage_ai_governance',minimumPlan:'business'}); if(!permission.ok)return permissionDeniedResponse(permission);
    ...
    let entity:{id:string;updated_at?:string}|null=null; let table:Parameters<typeof rollbackQmsCreate>[0]|null=null; let action=''; let entityType='';
    ...
    else if(workflow==='system_configure'){ const body=await parseJsonBodyWithZod(request,{schema:configureSchema,maxBytes:MAX_BYTES}); entity=await configureQmsSystem({organizationId:organization.id,actorUserId:user.id,...body}); action='qms_system_configured'; entityType='ai_qms_system'; }
    ...
    else if(workflow==='control_complete'){ const body=await parseJsonBodyWithZod(request,{schema:controlCompleteSchema,maxBytes:MAX_BYTES}); entity=await completeQmsControl({organizationId:organization.id,actorUserId:user.id,...body}); action='qms_control_completed'; entityType='ai_qms_control'; }
    ...
    else if(workflow==='nonconformity_close'){ const body=await parseJsonBodyWithZod(request,{schema:ncCloseSchema,maxBytes:MAX_BYTES}); entity=await closeQmsNonconformity({organizationId:organization.id,actorUserId:user.id,...body}); action='qms_nonconformity_closed'; entityType='ai_qms_nonconformity'; }
    ...
    else if(workflow==='audit_accept'){ const body=await parseJsonBodyWithZod(request,{schema:auditAcceptSchema,maxBytes:MAX_BYTES}); entity=await acceptQmsAudit({organizationId:organization.id,actorUserId:user.id,...body}); action='qms_audit_accepted'; entityType='ai_qms_audit'; }
    ...
    else if(workflow==='management_review_approve'){ const body=await parseJsonBodyWithZod(request,{schema:reviewApproveSchema,maxBytes:MAX_BYTES}); entity=await approveQmsManagementReview({organizationId:organization.id,actorUserId:user.id,...body}); action='qms_management_review_approved'; entityType='ai_qms_management_review'; }
    ...
    if(!event.persisted){
      if(table)await rollbackQmsCreate(table,organization.id,entity.id);
      if(workflow==='system_approve'&&entity.updated_at)await rollbackQmsApproval({organizationId:organization.id,qmsSystemId:entity.id,expectedUpdatedAt:entity.updated_at,actorUserId:user.id});
      return noStoreJson({error:'qms_audit_unavailable'},{status:503});
    }
