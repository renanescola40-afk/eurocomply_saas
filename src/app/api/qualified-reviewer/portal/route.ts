...
import { acceptReviewerInvite, getReviewerSession, revokeReviewerSession, saveReviewerAttestation } from '@/server/queries/qualified-reviewer-portal';
...
const attestationSchema = z.object({ sessionToken: z.string().regex(/^[a-f0-9]{64}$/), independenceConfirmed: z.boolean(), conflictDetails: z.string().trim().max(4000).nullable().optional(), scopeAcknowledged: z.literal(true) }).superRefine((value, ctx) => { if (value.independenceConfirmed && value.conflictDetails?.trim()) ctx.addIssue({ code: 'custom', message: 'conflict_requires_reassignment', path: ['conflictDetails'] }); });
...
export async function GET(request: Request) {
  try {
    ...
    if (!/^[a-f0-9]{64}$/.test(sessionToken)) return noStoreJson({ error: 'reviewer_session_required' }, { status: 401 });
    const limit = await checkDistributedRateLimit({ key: `qualified-reviewer-portal:get:${sessionToken.slice(0, 16)}`, limit: 30, windowMs: 60_000 });
...
export async function POST(request: Request) {
  try {
    ...
    const limit = await checkDistributedRateLimit({ key: `qualified-reviewer-portal:${workflow}:${request.headers.get('x-forwarded-for') ?? 'unknown'}`, limit: workflow === 'accept_invite' ? 8 : 20, windowMs: 60_000 });
    if (!limit.allowed) return denied(limit);
