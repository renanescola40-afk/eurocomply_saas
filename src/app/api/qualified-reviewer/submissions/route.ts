...
import { createReviewerSubmission } from '@/server/queries/qualified-reviewer-portal';
...
export async function POST(request: Request) {
  try {
    ...
    if (new Date(body.validUntil) <= new Date()) return noStoreJson({ error: 'review_validity_invalid' }, { status: 400 });
    const limit = await checkDistributedRateLimit({ key: `qualified-reviewer-submission:${body.sessionToken.slice(0, 16)}`, limit: 6, windowMs: 60_000 });
