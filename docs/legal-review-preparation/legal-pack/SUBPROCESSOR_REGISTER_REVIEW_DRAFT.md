...
|---|---|---|---|---|
| Vercel | Hosting, deployment, edge delivery and logs | traffic, deployment and diagnostic metadata | Live reconciliation on 2026-09-09: `www.risckcomply.com` resolves to Production deployment `dpl_GrWRgzmMnUfBattRgeBX4j3ic8LT`, state READY, `main@8a30ca3e6d06a77a83d7f2d10f7cef7edfe040a6`, target `production`; connected team is Pro; observed function region is `iad1`. This observed runtime region is not asserted as the complete contractual processing-location set. | applicable agreement/acceptance evidence, contractual processing locations, effective log-retention/add-on state, transfer treatment and qualified legal interpretation |
| Supabase | Database, authentication, storage and RLS | accounts, organisations, workflow content and audit events | Live reconciliation on 2026-09-09: Production project `tganhbbhfxcpblmgqprg` (`eurocomply_saas`) is `ACTIVE_HEALTHY` in `eu-west-1`; database reports PostgreSQL 17 GA. Standard DPA framework evidence exists from the prior assurance pack; account-specific superseding terms and retention options remain separate legal/account facts. | superseding agreement check, backup/PITR effective retention, transfer/subprocessor treatment and qualified legal interpretation |
...
| Sentry | Error monitoring and diagnostics | error context and diagnostic metadata | Current `/pt/trust` Production response on predecessor deployment evidence exposed `sentry-environment=production` and a release-bound SHA; the automatic Vercel build did not receive the protected Sentry auth token in that evidence set, so current release/source-map producer acceptance remains open; organisation region/DPA acceptance facts remain open | governed exact-SHA Sentry producer evidence, organisation region, plan/retention, DPA acceptance actor/timestamp, scrubbing/data categories, transfer terms and legal role |
...
| Upstash | Distributed rate limiting and security-abuse control state | operational request/control metadata and identifiers | Fresh predecessor-Production revalidation on 2026-08-24 showed canonical `/api/billing/catalog` traversing the Production fail-closed billing Redis limiter before a normal HTTP 200 response. Provider DPA/transfer/subprocessor framework is evidenced generally; exact-current post-release provider acceptance remains separate. | protected exact-SHA provider acceptance plus account owner/plan, region(s), retention/deletion, account-specific DPA applicability/acceptance, transfer treatment and counsel role decision |
...
- predecessor direct Production revalidation on 2026-08-24: `main@41cc6656de9a9d9df06b549dc1309d481498758b`, Vercel `dpl_FEUDn9oPpzetNwZcu3N5qJWmeAtZ`, `/api/billing/catalog` HTTP 200 and `/pt/trust` Sentry metadata bound to the same exact SHA.
- live Vercel reconciliation on 2026-09-09: `www.risckcomply.com` → `dpl_GrWRgzmMnUfBattRgeBX4j3ic8LT`, READY, target Production, `main@8a30ca3e6d06a77a83d7f2d10f7cef7edfe040a6`, observed function region `iad1`.
- live Supabase reconciliation on 2026-09-09: Production project `tganhbbhfxcpblmgqprg` ACTIVE_HEALTHY in `eu-west-1`.

The named repository evidence files retain predecessor-release provenance and do **not** receive current-release protected acceptance merely because a newer direct runtime revalidation exists. Direct current runtime signals establish factual binding only; they must not be interpreted as protected Provider Runtime acceptance, DPA acceptance, complete processing location, retention, transfer sufficiency or final provider role.
