...
| `src/app/api/internal/qualified-review-reminders/route.ts` | health/internal | Qualified-review reminder and expiry worker; requires internal cron authorization, pre-auth fail-closed rate limiting, server-owned scheduling, bounded processing, sanitized errors and no-store responses. |
| `src/app/api/internal/enterprise-access-operations/route.ts` | health/internal | Internal access-operations worker; requires cron/internal authorization, fail-closed authentication rate limiting, server-configured actor identity, bounded leased batches, tenant scope from persisted operation rows, sanitized errors and no-store responses. |
| `src/app/api/internal/enterprise-access-runtime/route.ts` | health/internal | Internal access-runtime SLO scheduler; requires cron/internal authorization, fail-closed authentication rate limiting, bounded tenant-scoped windows, sanitized errors and no-store responses. |
| `src/app/api/internal/enterprise-privileged-access-expiry/route.ts` | health/internal | Internal privileged-access expiry worker; requires fail-closed internal authentication rate limiting, cron authorization, bounded batch size, sanitized errors and no-store responses. |
| `src/app/api/internal/enterprise-access-notifications/route.ts` | health/internal | Internal access-escalation notification worker; requires fail-closed internal authentication rate limiting, cron authorization, leased bounded processing, sanitized provider failures and no-store responses. |
...
| `src/app/api/team/access-operations/route.ts` | admin-only | Tenant access operation creation and listing; authenticated organization context, `manage_team`, trusted mutation, fail-closed rate limiting, step-up, bounded JSON, tenant filters and no-store responses required. |
| `src/app/api/team/access-operations/[jobId]/route.ts` | admin-only | Tenant operation detail and pause/resume/cancel/retry controls; UUID validation, `manage_team`, tenant-filtered lookup, trusted mutation, fail-closed rate limiting, step-up, reason evidence and no-store responses required. |
| `src/app/api/team/access-operations/[jobId]/export/route.ts` | admin-only | Tenant member-level access-operation CSV export; authenticated organization context, `manage_team`, tenant-scoped RPC, hardened CSV helper, formula neutralization, filename sanitization, private no-store and nosniff headers required. |
| `src/app/api/team/access-runtime/route.ts` | admin-only | Tenant access-runtime dashboard, alert actions and asynchronous export creation; requires authenticated organization context, `manage_team`, trusted mutation, fail-closed rate limiting, step-up, bounded JSON, cursor pagination, tenant filters and no-store responses. |
| `src/app/api/team/privileged-access/route.ts` | admin-only | Tenant privileged-access request listing and creation; requires authenticated organization context, `manage_team`, trusted mutation, bounded JSON, fail-closed rate limiting, step-up authentication, server-derived tenant scope and no-store responses. |
| `src/app/api/team/privileged-access/[requestId]/decision/route.ts` | admin-only | Tenant privileged-access approval or rejection; requires UUID validation, authenticated organization context, `manage_team`, trusted mutation, fail-closed rate limiting, step-up authentication, separation of duties, tenant-scoped persistence and no-store responses. |
| `src/app/api/team/seat-contention/route.ts` | admin-only | Tenant seat-contention summary and reservation endpoint; requires authenticated organization context, `manage_team`, trusted mutation for POST, bounded JSON, fail-closed rate limiting, step-up authentication, server-derived tenant scope, optimistic contract-version checks and no-store responses. |
| `src/app/api/ai-governance/qualified-reviews/invites/route.ts` | high-risk | Qualified-review invitation issuance; requires authenticated organization context, `manage_ai_governance`, trusted origin, bounded Zod JSON, distributed fail-closed rate limiting, assignment/reviewer tenant matching, expiry capped at 14 days, digest-only token persistence, attributed audit evidence and no-store responses. |
| `src/app/api/ai-governance/qualified-reviews/invites/revoke/route.ts` | high-risk | Qualified-review invitation and session revocation; requires authenticated organization context, `manage_ai_governance`, trusted origin, bounded Zod JSON, distributed fail-closed rate limiting, tenant-scoped assignment validation, attributed audit evidence and no-store responses. |
| `src/app/api/ai-governance/qualified-reviews/control-center/route.ts` | tenant-scoped | Read-only qualified-review control center; requires authenticated organization context, `read_ai_governance`, distributed fail-closed rate limiting, server-derived tenant scope, security-invoker aggregation, sanitized errors and no-store responses. |
| `src/app/api/qualified-reviewer/portal/route.ts` | high-risk | External reviewer invite acceptance, reviewer-session workspace, attestation and logout; requires opaque digest-validated short-lived credentials, trusted origin for mutations, bounded Zod JSON, distributed fail-closed rate limiting, assignment/reviewer tenant binding, session expiry/revocation enforcement, sanitized errors and no-store responses. |
| `src/app/api/qualified-reviewer/submissions/route.ts` | high-risk | External reviewer opinion submission; requires opaque digest-validated reviewer session, trusted origin, bounded Zod JSON, distributed fail-closed rate limiting, prior independence and scope attestation, exact campaign SHA, terminal-state protection, append-only integrity evidence and no-store responses. |
...
| `src/app/api/ai-governance/qualified-reviews/route.ts` | high-risk | Qualified-review operational API; GET requires authenticated organization context and `read_ai_governance`, while mutations require trusted origin, bounded Zod JSON, distributed fail-closed rate limiting, `manage_ai_governance`, server-derived tenant scope, separation-of-duties checks, exact-SHA integrity, service-role-only lifecycle RPCs, append-only event evidence, sanitized exports and no-store responses. |
| `src/app/api/ai-governance/qualified-reviews/promote/route.ts` | high-risk | Exact-SHA qualified-review closeout promotion; requires authenticated organization context, `manage_ai_governance`, trusted origin, bounded Zod JSON, distributed fail-closed rate limiting, server-derived tenant scope, exact-SHA and evidence-digest validation, service-role-only atomic promotion, audit evidence and no-store responses. |
