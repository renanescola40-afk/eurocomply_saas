...
| Final validation runner | Open | `docs/security/evidence/runtime/final-validation-runner.json` or `docs/security/evidence/runtime/production-final-validation.json` must record the final runner output for the exact release commit | Release owner | Attach final runner output after all runtime checks pass |
| Production secrets configured in provider secret stores | Open | `docs/security/evidence/runtime/production-secrets-provider-stores.json` must prove required secrets are configured in provider secret stores without exposing values | Release owner | Confirm Vercel/GitHub/Supabase/Stripe/Sentry secrets in provider stores |
...
| External security review or pentest completed | Open | `docs/security/evidence/runtime/external-security-review-or-pentest.json` must record external review or an explicit approved release exception | Security reviewer | Attach external review evidence or approved exception before enterprise Go |
...
| Observability readiness | Open | `docs/security/evidence/runtime/observability-readiness.json` must prove Sentry/logging/smoke observability works without PII/secrets | SRE owner | Run observability readiness validation |
