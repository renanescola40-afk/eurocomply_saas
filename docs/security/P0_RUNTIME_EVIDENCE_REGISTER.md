...
| Supabase live RLS validation completed | Open | `supabase-live-rls-validation.json` must record status Complete, outcome passed, and tenant isolation evidence from the live script | Security reviewer | Required before production and enterprise Go |
...
| Upload scanning validation | Complete | `upload-malware-scan-validation.json` records Complete live provider proof artifact and fail-closed policy | Security reviewer | Revalidate before enterprise release or provider change |
