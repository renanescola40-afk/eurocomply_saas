...
- Current PR: #382
- Remediation branch: `security/upload-clamav-live-proof-2026-06-23`
- Current PR head SHA assessed by this register refresh: `90344010e993f80f495fd0d72f63ac7b0751ee4a`
...
| Audit-chain live validation | TargetValidationRequired | `docs/security/evidence/runtime/audit-chain-live-validation.json` must be regenerated by `scripts/security/run-audit-chain-live-validation.mjs` against the target Supabase project with migrations applied, RPC callable, normal append, concurrent stale previous-hash rejection plus retry, tamper detection, missing previous-hash detection, signed export readiness, and `AUDIT_CHAIN_LIVE_PROOF=true` reviewer confirmation. Repository-only evidence is not enough for enterprise. | Security reviewer | Required by 2026-06-25 for enterprise |
