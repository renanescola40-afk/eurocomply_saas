...
| Required status checks configured | Open | `docs/security/evidence/runtime/branch-protection-required-checks.json` must prove required checks are enforced on `main` | Release owner | Confirm every canonical required check and zero bypass actors for the exact SHA |
