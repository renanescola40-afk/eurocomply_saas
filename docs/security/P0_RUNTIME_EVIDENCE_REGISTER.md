...
| Current PR production deployment / build log | Complete | Vercel Ready preview and build log evidence observed for PR #431; this proves deployment presence only, not functional approval | Platform owner | Functional smoke verification still required |
...
| Final validation runner | Open | `node scripts/release/run-final-validation.mjs` must pass and attach summary/logs artifacts for all requested commands | Release owner | Required before Go |
...
| External security review or pentest completed | Open | `external-security-review-or-pentest.json` remains Open until a real external report or approved external review exists | Security reviewer | Required before enterprise/procurement |
...
| Audit-chain live validation | Exception | `audit-chain-live-validation.json` records target-live validation required; exception owner Security reviewer must keep enterprise blocked until the target Supabase run is complete | Security reviewer | Required before enterprise Go |
...
| Step-up MFA / IdP validation | Exception | `step-up-mfa-validation.json` records provider proof absent; exception owner Security reviewer must attach live provider proof before enterprise release | Security reviewer | Required before enterprise Go |
...
| Observability readiness | Complete | `observability-readiness.json` records health/ready controls, logging, alerting and owner governance as repository evidence | SRE / release owner | Attach deployment smoke, drill/sign-off and rollback verification |
| Incident response owner | Complete | Named incident owner evidence is recorded in the release approval record | Release owner | Attach acknowledgement/drill before Go |
| Rollback owner and rollback target | Exception | Named rollback owner and rollback target candidate are recorded; exception owner release owner must attach functional verification and dry-run evidence | Release owner | Required before Go |
