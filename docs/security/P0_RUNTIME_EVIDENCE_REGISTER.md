...
| Upload malware/content scanning validation | Complete | `docs/security/evidence/runtime/upload-malware-scan-validation.json` records status `Complete`, outcome `passed`, a real ClamAV provider proof, clean verdict, redacted provider metadata and no persisted provider response body/message | Security reviewer | Re-run if upload scanner provider, upload route, storage policy or scanner workflow changes |
