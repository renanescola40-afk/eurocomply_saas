...
Date: 2026-09-09  
Baseline: GDPR Chapter V. This register separates runtime location facts from legal transfer mechanisms.
...
|---|---|---|---|---|
| Supabase | Production project observed in `eu-west-1` | General DPA framework previously evidenced; account-specific superseding terms/transfer treatment open | BLOCKED | Confirm contractual entity, support/access locations, subprocessors, account-specific DPA/SCC/adequacy basis and backup/support access |
| Vercel | Observed function region `iad1`; managed global infrastructure | Contractual transfer treatment not frozen in current pack | BLOCKED | Confirm account agreement, processing/support locations, applicable adequacy/SCC framework and supplementary measures if required |
| Stripe | LIVE account country PT; global payments infrastructure | Provider contractual privacy framework exists generally but account/operator alignment is open | BLOCKED | Confirm contracting entity/account agreement and actual transfer basis for relevant data flows |
...
| Google Workspace | EMEA billed entity evidenced; corporate email operational | General provider CDPA/SCC framework exists | BLOCKED | Confirm account incorporation/acceptance, data-region/admin settings if relevant, onward transfers and legal role |
| GitHub / Actions | Operational use; hosted runners may transiently process Production data in protected workflows | Current account is a Personal User account; DPA treatment not final | BLOCKED | Confirm applicable agreement/entity, hosted-runner regions/access, transfer treatment and whether customer-facing disclosure is required |
...
| PostHog | EU endpoint binding proven; connected assurance project mismatch with Production | Account-specific DPA and project ownership unresolved | BLOCKED | Recover actual Production project/account and confirm region, retention, DPA and onward transfer facts |
...
The next closure action is provider-account evidence reconciliation, not generic legal drafting.