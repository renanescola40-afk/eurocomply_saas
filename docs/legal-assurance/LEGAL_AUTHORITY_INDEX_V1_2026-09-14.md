...
| Named qualified-review package artifacts listed by the current V2 matrix (eight AI Act workstream review files) | OPTIONAL_ASSURANCE_PREPARATION | Review packages remain available for a buyer, counsel or a future trigger; preparation is not accepted legal evidence. Founder facts, privacy, DPA and other launch controls are excluded from this optional row |
| docs/compliance/evidence/accepted/ (artifact-by-artifact) | EXTERNAL_DEPENDENCY | Qualified-review and optional-opinion artifacts are external evidence dependencies; `founder-facts.json` and other required launch evidence remain mandatory where the scorecard says so. Missing optional evidence does not prove that a qualified reviewer is legally required |
