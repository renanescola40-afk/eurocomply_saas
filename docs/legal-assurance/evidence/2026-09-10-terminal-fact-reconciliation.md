...
PRODUCTION_DOMAIN=www.risckcomply.com
...
The direct Production fetch of `/pt/privacy` and `/pt/terms` returned HTTP 200 and exposed the versioned `0.2-review` legal surfaces marked `REVIEW_DRAFT · HUMAN_REVIEW_REQUIRED`.

The main/Production difference is a later merged CI/release-diagnostic change. This supports content continuity for the observed legal pages but does not claim exact-current-main Production identity.
...
SIGNATORY_AUTHORITY=OPEN_WHERE_REQUIRED
...
- Supabase describes itself as processor for the hosted service under its shared-responsibility model;
- from 2026-08-01, its DPA is incorporated into its Terms of Service for customers and a separately executed DPA is no longer required for that general route;
- separately negotiated agreements remain binding;
- additional transfer/TIA resources are available through Supabase legal resources / Studio.
...
```text
PostHog DPA — Samuel Cerqueira, Unipessoal, Lda
