...
Record current Legal Assurance truth without converting source work, CI, owner decisions, provider statements or AI analysis into qualified legal acceptance.
...
PRODUCTION_FUNCTION_REGION=iad1
PRODUCTION_MAIN_RELATION=CURRENT_MAIN_AHEAD_OF_OBSERVED_PRODUCTION_RELEASE
PROTECTED_MAIN=true
```

Direct connected validation on 2026-09-12 confirms the canonical Production deployment remains the Beagle-observed deployment at SHA `13b19410...`. Protected `main` is ahead and is not claimed to equal that observed Production release.
...
The 80/50 working scores remain conservative because the repository has no canonical weighting model that would justify inventing a new aggregate percentage. The seller/contracting decision is now closed, but that factual improvement is not converted into a synthetic percentage change.
...
| Data Governance Runtime V2 | PASS_EXACT_SHA | Historical exact-SHA technical proof retained |
| Public Privacy review structure | PASS_CANONICAL_MAIN_AND_LIVE_REVIEW_SURFACE | Registry/provider/legal-basis/DPO/qualified-review gates remain |
| Public DPA review structure | PASS_CANONICAL_MAIN | Registry, provider/transfer facts and qualified Art. 28 review remain |
...
| Public Terms review structure | PASS_CANONICAL_MAIN_AND_LIVE_REVIEW_SURFACE | Final risk allocation, VAT/registry facts and qualified review remain |
| Public claims guard posture | PASS_SAFE_BOUNDARY | Continue blocking unsupported certification/compliance guarantees |
| RISCK COMPLY operator entity | OWNER_DESIGNATED | `SAMUEL CERQUEIRA, UNIPESSOAL LDA` |
| RISCK COMPLY customer contracting entity | OWNER_DESIGNATED | `SAMUEL CERQUEIRA, UNIPESSOAL LDA` |
| RISCK COMPLY seller entity | OWNER_DESIGNATED | `SAMUEL CERQUEIRA, UNIPESSOAL LDA` |
| SaaS CAE / activity association | DEFERRED_BY_OWNER | Final administrative phase; no change authorised now |
| Provider DPA factual evidence | PARTIAL_ADVANCED | Supabase/PostHog framework facts advanced; account/transfer residuals remain |
| VAT/seller tax model | OPEN_EXTERNAL_FACT | Seller selected; authoritative registry/VAT and Stripe reconciliation still required |
| Master Legal Opinion | OPEN_EXTERNAL | Only after valid 8/8 workstream outcomes and material dependencies |

## Historical exact-SHA Data Governance proof

Previously accepted technical proof remains retained for its tested SHA:
...
The pages are deliberately review drafts. Final/effective publication remains blocked until required authoritative facts and qualified decisions exist.

## Entity facts — owner decision closed

On 2026-09-12 the owner explicitly designated the same existing Portuguese company as RISCK COMPLY operator, customer contracting entity and seller:
...
This closes the prior question of **which company** will contract with and invoice customers. The exact registered office, company/tax identifiers, VAT regime/registrations and signatory facts still require authoritative evidence before final publication.
...
No lane may create, infer or alter a CAE merely to make legal, billing, procurement or assurance gates green.
...
| Contracting/seller entity | SAMUEL CERQUEIRA, UNIPESSOAL LDA | OWNER_SELECTED / REGISTRY_TAX_COUNSEL_EVIDENCE_PENDING |

These selections reduce owner-decision uncertainty but do not create binding Terms. Material remaining owner/counsel decisions include liability cap/carve-outs, indemnities, breach cure/suspension mechanics, legal-notice mechanics and any positive warranty/remedy position.

## Provider factual evidence

```text
VERCEL_PRODUCTION_DEPLOYMENT=READY_SHA_13b19410_AT_2026_09_12_OBSERVATION
...
POSTHOG_DPA_COMPLETION_EVIDENCE=PRESENT_FOR_OWNER_DESIGNATED_SELLER_ENTITY
...
PROVIDER_FACTUAL_CLOSURE=PARTIAL_ADVANCED
```

Provider framework/DPA evidence does not independently prove all applicable transfer mechanisms, locations, retention, account-specific project binding or qualified legal sufficiency.
...
- authoritative registered facts for the already-selected operator/contracting/seller entity;
...
Source completion must not be collapsed into final legal acceptance.
...
1. authoritative registry evidence for `SAMUEL CERQUEIRA, UNIPESSOAL LDA`, including registered facts required for final legal instruments;
2. Portuguese VAT regime, registrations and supported B2B sales matrix through attributable tax/accounting authority;
3. remaining account-specific provider DPA/processing/retention/transfer facts, especially actual PostHog Production account/project attribution;
...
8. deferred SaaS CAE/activity administrative reconciliation at the owner-selected final phase;
9. deliberate final legal publication only after all required upstream gates pass.

## Current shortest path

```text
CONTRACTING_SELLER_ENTITY_OWNER_DESIGNATION=PASS
...
→ FINAL_CAE_ACTIVITY_ADMINISTRATIVE_RECONCILIATION
→ FINAL_PUBLICATION
```
