...
**Subject baseline:** `main@d4549aa92938f2ed9bfb0de7cac884a70f80e187`  
...
| --- | --- | --- | --- | --- | --- |
| Vercel | Connected Vercel account contains project `eurocomply-saas`; the latest target `production` deployment is `READY`, aliases include `risckcomply.com` and `www.risckcomply.com`, and deployment metadata binds it to exact current `main@d4549aa92938f2ed9bfb0de7cac884a70f80e187`. | Current production deployment reports function region `iad1`. This is a runtime location signal, not a legal transfer conclusion. | Current public DPA: `https://vercel.com/legal/dpa`. It describes processor terms, subprocessors and transfer mechanisms for covered agreements/plans. | `CURRENT_RUNTIME_ENABLED / LEGAL_FACTS_INCOMPLETE` | Confirm actual account plan/agreement, contracting entity, DPA applicability/acceptance, relevant subprocessor snapshot, retention/deletion terms, and approved transfer treatment. |
...
| Sentry | Current production HTML revalidated on 2026-08-20 contains Sentry trace metadata with `sentry-environment=production` and `sentry-release=d4549aa92938f2ed9bfb0de7cac884a70f80e187`; the production CSP also permits Sentry ingestion domains. This is current runtime evidence that Sentry observability is active for the deployed release. | Sentry publicly supports US and Germany data-storage locations, but the current RISCK COMPLY Sentry organization region is not established by the evidence retained here. | Current Sentry documentation confirms region-specific US/DE hosting/API domains; account-level DPA/subprocessor/retention material still requires reconciliation. | `CURRENT_RUNTIME_ENABLED / LEGAL_FACTS_INCOMPLETE` | Confirm organization/project ownership, actual organization region, provider legal entity, scrubbing/data categories, retention, DPA/subprocessor/transfer terms, and account applicability. Runtime configuration changes remain Chat 1-owned. |
| PostHog | Current production delivery revalidated on 2026-08-20 still includes the application's `PostHogAnalyticsProvider`, EU ingestion/assets endpoints in the production CSP, and the existing consent-gated analytics architecture. Exact client/project identifiers are intentionally omitted. | Runtime configuration is explicitly pointed at PostHog's EU service endpoints. | PostHog's current Trust Center publishes a DPA, subprocessor material and privacy/security evidence; exact account applicability remains separate. | `CURRENT_RUNTIME_CONFIGURED / CONSENT_GATED` | Confirm PostHog account/project ownership, contractual entity, DPA/account applicability, project retention, exact data/event categories, consent-policy legal approval, and transfer/subprocessor treatment. Do not infer that every visitor transmits events merely because the provider is configured. |
| Resend / transactional email | Canonical repository code and `.env.example` support Resend-backed email delivery. A scoped current-production log search in the prior snapshot returned no matching `resend` log entries; absence of logs is not absence proof. | Current RISCK COMPLY account/runtime region is unproven. Resend's public DPA states its primary processing operations take place in the United States. | Current public DPA: `https://resend.com/legal/dpa`; current public subprocessors: `https://resend.com/legal/subprocessors`. Public terms identify Plus Five Five, Inc. and describe DPA/SCC/subprocessor treatment and deletion after termination, but do not prove RISCK COMPLY account usage or acceptance. | `CONDITIONAL_UNVERIFIED / PROVIDER_PUBLIC_MATERIAL_AVAILABLE` | Confirm whether Resend is actually enabled in production and, only if active, establish account/legal entity, applicable agreement/DPA, message data, retention, subprocessor/transfer terms and customer-notice implications. |
...
The 2026-08-19 provider snapshot at `main@3ad7a6c5c11a69b9c7f61115672278b05e84f984` is also historical for release-bound runtime facts. It remains useful provenance, but current Vercel/Sentry/Stripe statements in this register are bound to the 2026-08-20 reconciliation above.
