...
**Baseline protected main for this reconciliation:** `7c5f344d3049ddb3e65003e5f7e7eaae670f0538`  
**Current retained Production runtime subject:** `7c5f344d3049ddb3e65003e5f7e7eaae670f0538`  
**Current retained Production deployment:** `dpl_HjY8HsY874YjSDzUs9m1z9yxqWpC`  
**Canonical tracker:** GitHub issue `#1727`

Protected-main lineage and Production runtime lineage are separate authorities even when, as in this snapshot, they resolve to the same SHA.
...
## Retained exact-current runtime evidence
...
| --- | --- | --- | --- | --- |
| Vercel | Production deployment `dpl_HjY8HsY874YjSDzUs9m1z9yxqWpC` is `READY`, target `production`, ref `main`, and attributable to `7c5f344d...`; connected team plan is `pro`. Current Vercel DPA expressly covers Pro and Enterprise customers and includes SCC/subprocessor mechanics. | Runtime placement is deployment-specific and does not establish all processing locations. Standard Pro Runtime Logs retention is 1 day; Observability Plus would extend it to 30 days, but the account add-on state is not exposed here. | `CURRENT_RUNTIME_PROVEN / PRO_PLAN_PROVEN / STANDARD_DPA_PRO_PLAN_MATCHED / SCC_FRAMEWORK_PROVEN` | Observability Plus state if material; account-specific acceptance actor/timestamp or custom/superseding agreement; final legal role/transfer interpretation. |
...
| Google Workspace | Corporate mailboxes on `risckcomply.com` are actively used for support, security, procurement and legal communications. Account-specific billing evidence proves Google Workspace Business Starter with 2 licenses and identifies Google Cloud EMEA Limited as the billed EMEA provider entity. Provider-published Workspace data-processing terms establish a general CDPA/SCC framework. | Exact account data region, effective retention settings and onward-transfer treatment are not established by the retained billing evidence. | `ACCOUNT_SERVICE_PROVEN / BUSINESS_STARTER_PROVEN / EMEA_ENTITY_PROVEN / CDPA_SCC_FRAMEWORK_PROVEN_GENERAL / ACCOUNT_INCORPORATION_OPEN` | Confirm the applicable Workspace agreement/CDPA incorporation or opt-in state for this exact account, admin-configured retention/data-region settings if material, and qualified legal role/transfer interpretation. |
...
| Sentry | Retained canonical Production observation on deployment `dpl_HjY8...` exposes `sentry-environment=production` and exact `sentry-release=7c5f344d...`; the redacted evidence artifact is retained in-repository. Human support established the self-service assurance-material boundary. | Public runtime evidence does not expose a deterministic US-vs-Germany organization storage region. | `CURRENT_RELEASE_BINDING_PROVEN / RETAINED_EVIDENCE / HUMAN_SUPPORT_BOUNDARY_PROVEN / ACCOUNT_DPA_REGION_OPEN` | Owner-visible organization region, plan/retention and DPA acceptance actor/timestamp; final transfer/legal interpretation. |
...
| Upstash / Redis | Canonical `GET /api/billing/catalog` returned HTTP 200 on Production deployment `dpl_HjY8...` / SHA `7c5f344d...`. The route calls the distributed billing rate limiter before returning the catalogue; the Production billing policy is high-risk/fail-closed, and Redis not configured/request failed/unavailable cannot yield the normal allowed response. A redacted evidence artifact is retained in-repository. Provider-published Terms/DPA establish `Upstash, Inc.`, incorporated DPA, transfer/subprocessor framework and deletion-on-termination mechanics. | Exact account region/plan remain unproven; no fixed account-specific retention period is inferred from generic documents. | `CURRENT_EXACT_RELEASE_RUNTIME_PROVEN / RETAINED_EVIDENCE / PROVIDER_FRAMEWORK_PROVEN_GENERAL / ACCOUNT_FACTS_OPEN` | Account plan/owner/region; account-specific agreement/DPA acceptance; final retention/transfer/legal interpretation. |
...
- `SENTRY_CURRENT_RELEASE_BINDING_OPEN` — resolved for runtime binding on `7c5f344d...`; organization region/DPA acceptance remain separate.
...
- `UPSTASH_CURRENT_EXACT_RELEASE_REPROOF_OPEN` — resolved by retained redacted canonical `/api/billing/catalog` proof on Production `7c5f344d...`; account/legal facts remain separate.
...
Protected-main lineage and Production runtime lineage are separate authorities. Release-specific facts in this register are bound to the stated retained Production runtime subject. Documentation-only commits do not by themselves invalidate provider/account facts. A material runtime, provider configuration, data-flow, region, retention or service-scope change requires revalidation.
...
`SENTRY_CURRENT_EXACT_RELEASE_RUNTIME: PROVEN`
