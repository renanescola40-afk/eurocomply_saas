...
**Latest runtime evidence subject captured:** `b54afdfd6370442e7c7924f31d6210841621cf38`  
**Canonical tracker:** GitHub issue `#1727`  
**Current connector-observation ledger:** `docs/trust/evidence/2026-08-21-external-assurance-runtime-provider-revalidation.md`
...
| Sentry | Live production HTML captured on 2026-08-21 from `/pt/trust`, `/pt/security`, and `/pt/status` for runtime evidence subject `b54afdfd6370442e7c7924f31d6210841621cf38` contained `sentry-environment=production` and `sentry-release=b54afdfd6370442e7c7924f31d6210841621cf38`; the captured CSP permitted Sentry ingestion domains. | Sentry publicly supports US and Germany data-storage locations, but the RISCK COMPLY organization region is not established by the evidence retained here. | Sentry documentation confirms region-specific US/DE hosting/API domains; account-level DPA/subprocessor/retention material still requires reconciliation. | `RUNTIME_EVIDENCE_CAPTURED / LEGAL_FACTS_INCOMPLETE` | Confirm organization/project ownership, actual organization region, provider legal entity, scrubbing/data categories, retention, DPA/subprocessor/transfer terms, and account applicability. Runtime configuration changes remain Chat 1-owned. |
| PostHog | Retained production delivery evidence from 2026-08-20 contained the application's `PostHogAnalyticsProvider` and `AnalyticsConsentBanner` components, while the captured CSP permitted EU PostHog ingestion/assets endpoints. This row intentionally remains bound to that earlier attributable capture rather than promoting the 2026-08-21 release observation. These facts do **not** establish that the production PostHog project key is populated, that the consent-required flag is enabled, or that events are transmitted. Exact client/project identifiers are intentionally omitted. | The retained 2026-08-20 CSP capture permitted PostHog EU service endpoints, but endpoint allowlisting alone does not prove active processing or account region. | PostHog's Trust Center publishes a DPA, subprocessor material and privacy/security evidence; exact account applicability remains separate. | `RUNTIME_COMPONENT_PRESENT / ENABLEMENT_AND_CONSENT_UNVERIFIED` | Confirm whether the production PostHog project key is populated and whether consent-required mode is enabled; if active, confirm account/project ownership, contractual entity, DPA/account applicability, project region and retention, exact data/event categories, consent-policy legal approval, and transfer/subprocessor treatment. |
...
| OpenAI / ChatGPT — founder operational use outside SaaS runtime | Owner declaration on 2026-08-20 identifies ChatGPT/OpenAI as the only external AI/model service used operationally. Repository/configuration review has not identified a direct model-provider integration in the SaaS runtime. Owner-provided screenshot confirms the personal ChatGPT control `Melhorar o modelo para todo mundo` is enabled. Where customer content is manually supplied to that workspace, OpenAI's current consumer documentation states such new conversations may be used to improve/train models while the control remains enabled. | Current RISCK COMPLY-specific processing/storage location for this personal workspace is not established by retained evidence. | Consumer data controls: `https://help.openai.com/en/articles/8983130-how-does-chatgpt-use-my-data`; EU privacy policy: `https://openai.com/policies/eu-privacy-policy/`; business-data commitments: `https://openai.com/business-data/`. OpenAI states Business/Enterprise/API inputs and outputs are not used for training by default, which must not be projected onto the current personal workspace. | `FOUNDER_FACT_CAPTURED / PERSONAL_WORKSPACE_MODEL_IMPROVEMENT_ENABLED / DIRECT_RUNTIME_INTEGRATION_NOT_IDENTIFIED / LEGAL_FACTS_INCOMPLETE` | Confirm the exact account/workspace terms and retention/deletion posture applicable to the current operational use. Qualified counsel must decide the legal role and whether customer content may be processed through this personal workspace, requires a Business/Enterprise/API arrangement, or must be prohibited from this workflow. Do not claim a DPA, processor role, no-training posture, or compliant transfer treatment without attributable evidence and legal approval. |
...
Earlier runtime snapshots at `3ad7a6c5c11a69b9c7f61115672278b05e84f984`, `d4549aa92938f2ed9bfb0de7cac884a70f80e187`, `127aed7e954d75ea71f20cfbeb0fb3ed4212615c`, and `e8f2d10d6fe03096b07eeb9a049742cd049cc78b` remain provenance for their respective capture points. The latest release-bound runtime observation recorded in this register is subject `b54afdfd6370442e7c7924f31d6210841621cf38`; the commit that contains this document is intentionally not treated as proof of deployment.
