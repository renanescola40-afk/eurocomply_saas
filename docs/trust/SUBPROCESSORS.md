...
| Sentry | Error monitoring and diagnostics if enabled | Error context and diagnostic metadata | Repository supports Sentry. Historical provider-store inventory is stale for current enablement because it is dated 2026-06-21 and bound to old SHA `8389985112ec23f0e058a75a48b789a5f7a9b8d6`. | Needs current revalidation |
| PostHog | Product analytics if enabled | Consented analytics events and identifiers | Canonical configuration supports PostHog EU endpoints, but current production enablement is not proven by this register. | Conditional / unverified |
