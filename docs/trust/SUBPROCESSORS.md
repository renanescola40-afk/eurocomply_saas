...
| PostHog | Product analytics | Analytics events and identifiers if enabled | Retained production delivery evidence from 2026-08-20 contained the PostHog analytics/consent components and the captured CSP permitted EU PostHog ingestion/assets endpoints. This row intentionally remains bound to the prior attributable capture and does **not** promote the 2026-08-21 release observation. It also does **not** prove that the production project key is populated, that consent-required mode is enabled, or that events are transmitted. | Runtime component present / enablement and consent unverified |
