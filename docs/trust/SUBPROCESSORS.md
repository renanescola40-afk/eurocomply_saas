...
| --- | --- | --- | --- | --- |
| Vercel | Application hosting and deployment | Application traffic, deployment metadata, logs | Connected project `eurocomply-saas`. At evidence capture, repository `main` was `127aed7e954d75ea71f20cfbeb0fb3ed4212615c` and its target-production deployment was still `BUILDING`; the latest completed target-production deployment remained `READY` at `d4549aa92938f2ed9bfb0de7cac884a70f80e187`. Region signal is `iad1`. Contract/DPA applicability and transfer treatment remain unresolved. | Current production READY on prior main / current-main deployment in progress / legal facts incomplete |
...
| Stripe | Billing and subscription management | Billing metadata; payment details handled by Stripe | Connected Stripe account now reports display name `RISCK COMPLY SAAS`. PR `#1732` established the reviewed new account as intended LIVE provider authority and PR `#1734` hardened the Vercel secret-type/authority path, but these merges alone do not prove successful post-merge activation/bootstrap, webhook/Vercel rebinding, provider runtime proof, or genuine customer billing lifecycle completion. Exact contracting entity, DPA/account applicability, region/transfer and retention details remain unresolved. Billing runtime remains Chat 3-owned. | Account identity reconciled / LIVE runtime cutover unproven / legal facts incomplete |
...
| Sentry | Error monitoring and diagnostics | Error context and diagnostic metadata | The latest completed production release remained `d4549aa92938f2ed9bfb0de7cac884a70f80e187` at evidence capture; production HTML had been revalidated on 2026-08-20 with Sentry `environment=production` and release metadata matching that READY SHA, and CSP permits Sentry ingestion. The newer `main@127aed7e...` deployment was still BUILDING, so no live Sentry-release claim is made for it. | Current runtime enabled on latest READY release / current main not yet proven live / legal facts incomplete |
| PostHog | Product analytics | Analytics events and identifiers if enabled | Current production delivery revalidated on 2026-08-20 includes the application's PostHog analytics/consent components, and the production CSP permits EU PostHog ingestion/assets endpoints. This does **not** prove that the production project key is populated, that the consent-required flag is enabled, or that events are currently transmitted. | Runtime component present / enablement and consent unverified |
| Resend / email provider | Transactional and support email | Email address, message metadata and content | Repository supports Resend-backed delivery. A prior scoped current-production log search returned no `resend` matches, which is not absence proof. Resend publishes current DPA/subprocessor material, but actual production enablement and RISCK COMPLY account applicability remain unverified. | Conditional / unverified; provider public material available |
...
| AI/model provider used by RISCK COMPLY | Optional AI-assisted features | Prompts, outputs and permitted customer content | No dedicated AI/model-provider configuration group or direct common provider integration was found in the prior repository baseline. This is not production-absence proof; authorised-officer confirmation is still required. | Founder fact required |
