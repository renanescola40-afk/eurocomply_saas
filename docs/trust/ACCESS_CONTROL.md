...
Enterprise SAML SSO runtime is implemented and production schema/RPCs are present; buyer-specific IdP/domain activation and end-to-end validation remain external. Tenant-wide MFA is available as an organization policy: when enabled, AAL2 is required across licensed workspace pages, shared API/RBAC authorization and tenant-scoped RLS authority. Step-up MFA/AAL2 also remains enforced for protected administration and high-risk actions. Fine-grained custom roles are not currently documented as available beyond the implemented Trust Center RBAC model, and legacy owner/admin/member-only flows must be treated as migration gaps until they explicitly use the shared RBAC helper.

## Customer-safe answer

Use: "RISCK COMPLY implements organization-scoped roles and permissions for owner, admin, member, and Trust Center RBAC-model editor/viewer access where flows use the shared RBAC helper, backed by server-side membership checks and Supabase RLS migrations. Some legacy server paths remain owner/admin/member-only and must be described within that implemented role model. Enterprise SAML SSO runtime is implemented; activation requires buyer-specific IdP/domain configuration and end-to-end validation. Step-up MFA/AAL2 is enforced for protected administration and high-risk actions, and tenant-wide mandatory MFA is implemented as a tenant-configurable AAL2 policy. Activation for a specific customer tenant must be stated from that tenant's actual configuration."
