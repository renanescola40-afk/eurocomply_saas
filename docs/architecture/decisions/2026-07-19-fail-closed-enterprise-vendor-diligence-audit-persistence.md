# Fail closed on enterprise vendor-diligence audit persistence

- **Date:** 2026-07-19
- **Status:** Proposed
- **Priority:** P1 — third-party AI governance, accountability, audit integrity

## Context

`POST /api/ai-systems?workflow=vendor_due_diligence` creates an organization-scoped enterprise vendor due-diligence record and then emits `vendor_due_diligence_started` through the durable audit writer. The route previously ignored the writer's explicit `persisted` result and returned HTTP 201 even when the audit event could not be stored.

That behavior could leave an active third-party AI governance review without durable evidence identifying who initiated it and under which organization. This finding is based on repository control flow only. It does not assert a production incident, exploit, penetration test, external audit result, or customer impact.

## Decision

The workflow must require `audit.persisted === true` before returning the created vendor review.
...
The route then returns a no-store HTTP 503 response with the stable error code `vendor_diligence_audit_unavailable`. A compensation failure is reported only with a fixed event name and sanitized provider code.
...
Revert the commits in the pull request. No database migration, data migration, dependency rollback, secret rotation, or infrastructure rollback is required. Existing vendor due-diligence rows are not modified by the rollback.
