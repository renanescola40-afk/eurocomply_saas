...
| AI Inventory create | Complete | User can create AI systems through the inventory UI and API with owner, category, market, processed data, vendor/model, use case, lifecycle status and risk signals. | Apply the new Supabase migration in production. |
...
| AI Inventory edit/reassessment | Partial | Backend `PATCH /api/ai-systems/:id` validates organization permission, recalculates classification, updates the record, writes history and audit metadata. | Add a full edit form on the detail page using the existing API. |
...
| Audit Timeline | Partial | Audit logs exist and AI system create/reassessment writes audit events. Dashboard previews audit events. | Add a dedicated organization timeline page with actor display and safe metadata rendering. |
...
| Important activity is auditable | Partial/usable; AI inventory create/reassessment and task actions write audit events. |
...
1. Add full edit/reassessment UI on `/{locale}/ai-systems/{id}` using the implemented PATCH endpoint.
2. Add database access policies for `ai_system_history` if the app ever reads it outside the server admin boundary.
3. Connect AI system records to vendors, documents, risks and tasks via explicit relationship fields.
4. Add dedicated organization audit timeline route.
5. Expand new AI inventory labels to every supported locale.
6. Add tests for AI inventory create/detail/reassessment and route-level permission denial.
