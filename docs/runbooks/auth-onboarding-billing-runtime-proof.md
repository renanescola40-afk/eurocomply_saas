...
The separate **Stripe Provider Proof** must also be green before commercial launch. It validates the live account, canonical live monthly prices, exact production webhook contract, and the active live Billing Portal configuration used by runtime sessions. `config/stripe-billing-portal-contract.json` is the single configuration authority compiled into the same release SHA: `configurationId: null` intentionally uses the active live account default, while a reviewed non-null `bpc_...` value pins that exact configuration. `config/stripe-billing-portal-policy.json` separately defines the reviewed provider features and keeps direct provider subscription update/cancel disabled. The protected provider proof reads both checked-out files, so GitHub/Vercel environment drift cannot select a different portal configuration and undocumented provider feature drift cannot silently pass. Retained evidence records only whether the binding mode was `explicit` or `default`; it does not retain the configuration ID. See `docs/decisions/ADR-0092-versioned-stripe-billing-portal-authority.md` and `docs/runbooks/stripe-billing-portal-bootstrap.md`. A runtime organization proof does not substitute for provider configuration readiness.
...
- provider proof `billingPortalConfigurationPresent`: provision an active live Customer Portal configuration. Prefer the protected `Stripe Billing Portal Bootstrap` when no usable configuration exists;
...
- provider proof `billingPortalConfigurationPolicyMatches`: compare the selected Stripe object with `config/stripe-billing-portal-policy.json`. Do not enable direct Stripe Portal subscription update/cancel to make the proof pass; reconcile provider state through the reviewed bootstrap/change-control path;
...
The Billing Portal authority rollback is documented in ADR-0092. Setting the versioned contract back to `configurationId: null` through a reviewed PR restores account-default selection after that exact SHA is deployed. A bootstrap-created provider object must not be deactivated while any deployed release pins it.
