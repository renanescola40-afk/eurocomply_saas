...
Complete the authorized repository task, create or update the working branch, commit the coherent changes, and open or update the pull request even when Vercel reports an external quota or rate-limit condition such as:
...
This prompt permits pull-request delivery despite an external Vercel quota condition. It does not permit faking checks, removing Vercel from required protections, weakening CI, automatic branch synchronization, auto-merge, merging through a failed required check, or claiming production evidence that does not exist.
