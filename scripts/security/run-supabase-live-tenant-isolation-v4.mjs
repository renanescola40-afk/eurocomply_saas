...
async function setup(admin, created) {
  ...
  const seeds = {
    ...
    audit_events: await seed(admin, 'audit_events', { organization_id: org.B.id, actor_id: user.ownerB.id, actor_user_id: user.ownerB.id, action: 'seeded_event', entity_type: 'rls_validation', entity_id: suffix }, created),
...
function spec(table, c) {
  ...
  return ({
    ...
    audit_events: { seed: c.seeds.audit_events, insert: { organization_id: id, actor_id: u.ownerA.id, actor_user_id: u.ownerA.id, action: 'cross', entity_type: 'proof', entity_id: s }, sameDenied: { organization_id: id, actor_id: u.ownerB.id, actor_user_id: u.ownerB.id, action: 'same', entity_type: 'proof', entity_id: s }, update: { action: 'mutated' } },
