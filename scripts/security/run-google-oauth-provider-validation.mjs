...
const failures = [];
for (const name of required) if (!String(process.env[name] ?? '').trim()) failures.push(`missing_${name.toLowerCase()}`);
...
if (failures.length === 0) {
  try {
    const response = await fetch(`https://api.supabase.com/v1/projects/${encodeURIComponent(process.env.SUPABASE_PROJECT_REF)}/config/auth`, {
    ...
    siteUrlConfigured = typeof config.site_url === 'string' && /^https:\/\//.test(config.site_url);
    ...
    redirectAllowlistConfigured = allowlist.some((value) => /^https:\/\//.test(value) && value.includes('/auth/callback'));
    providerHost = new URL(config.site_url).hostname;
    if (!providerEnabled) failures.push('google_provider_disabled');
    if (!siteUrlConfigured) failures.push('site_url_not_https');
    if (!redirectAllowlistConfigured) failures.push('callback_allowlist_missing');
  } catch (error) {
    failures.push(error instanceof Error ? error.message.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 80) : 'provider_validation_failed');
  }
}
...
const evidence = {
  ...
  generatedAt: now,
  summary: passed
    ? 'Protected Supabase Management API validation confirmed that Google OAuth is enabled and the production auth redirect configuration is present for the exact release SHA.'
  ...
  checks: {
    ...
    callbackAllowlistConfigured: redirectAllowlistConfigured,
  },
  failures,
  controlsVerified: passed ? ['google_oauth_provider_enabled', 'oauth_site_url_configured', 'oauth_callback_allowlist_configured'] : [],
  evidenceLocations: [
  ...
  productionGate: passed ? 'eligible for final release evaluation' : 'blocked until protected Google OAuth provider validation passes',
