...
export const APPROVED_EVIDENCE_OVERRIDES = Object.freeze({
  'TRU-01': Object.freeze({
    path: 'artifacts/trust-claims/trust-claims-validation.json',
    check: 'publicClaims',
  }),
  'TRU-02': Object.freeze({
    path: 'artifacts/trust-claims/trust-claims-validation.json',
    check: 'publicClaims',
  }),
  'TRU-03': Object.freeze({
    path: 'artifacts/trust-claims/trust-claims-validation.json',
    check: 'publicClaims',
  }),
});
...
export function validateEvidenceOverrides(config, document) {
  ...
  if (!exactKeys(document, ['schemaVersion', 'overrides'])) {
    failures.push('evidence overrides top-level fields must be exactly schemaVersion and overrides');
  }
  ...
  if (document.overrides.length !== approvedEntries.length) {
    failures.push(`evidence overrides must contain exactly ${approvedEntries.length} approved entries`);
  }
  ...
  for (const entry of document.overrides) {
    ...
    if (!availableControls.has(controlId)) {
      failures.push(`evidence override references unknown control: ${controlId}`);
    }
    ...
    if (entry.evidence.path !== approved.path) {
      failures.push(`${controlId} evidence.path is not approved`);
    }
    if (entry.evidence.check !== approved.check) {
      failures.push(`${controlId} evidence.check is not approved`);
    }
  }
