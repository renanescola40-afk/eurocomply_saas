...
if (!/^\d+$/.test(supabasePromotionRunId)) {
  throw new Error('SUPABASE_PROMOTION_RUN_ID must bind TEN-RLS to the successful exact-SHA governed Production promotion');
}

const workflows = [
  { file: 'auth-rbac-runtime-proof.yml', controls: ['IAM-01','IAM-02','IAM-03','IAM-04','IAM-05','IAM-06','TEN-01'], inputs: { release_sha: targetSha } },
  { file: 'supabase-live-rls-validation.yml', controls: ['TEN-02','TEN-03','TEN-04','TEN-05','TEN-06'], inputs: { release_sha: targetSha, promotion_run_id: supabasePromotionRunId, confirmation: 'EXECUTE_POST_FORWARD_PROMOTION_RUNTIME_PROOF' } },
...
const receipt = {
  ...
  evidenceBoundary: 'A dispatch receipt proves orchestration only. TEN-RLS is bound to the exact successful governed Production promotion; controls remain open until protected workflows emit passing exact-SHA evidence.',
};
