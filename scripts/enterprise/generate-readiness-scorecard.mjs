...
export function resolveEvidenceDocuments({ primaryDocument, primaryCheck, repositoryDocument, repositoryCheck }) {
  const primaryStatus = evaluateEvidenceDocument(primaryDocument, primaryCheck);
  if (primaryStatus !== STATUS.NOT_VERIFIED) return { status: primaryStatus, source: 'configured_evidence' };
  const repositoryStatus = evaluateEvidenceDocument(repositoryDocument, repositoryCheck);
  if (repositoryStatus !== STATUS.NOT_VERIFIED) return { status: repositoryStatus, source: 'repository_ci' };
...
export function createEvidenceReader(repositoryEvidencePath = DEFAULT_REPOSITORY_EVIDENCE) {
  ...
  return (evidence, controlId) => {
    ...
    if (primaryResult.kind === 'invalid') return { status: STATUS.FAIL, reason: 'evidence_file_invalid_json', source: 'configured_evidence' };
    ...
    if (resolved.source === 'configured_evidence') {
      return { status: resolved.status, reason: evidence.check ? `derived_from_check:${evidence.check}` : 'derived_from_document_status', source: resolved.source };
    }
    if (resolved.source === 'repository_ci') {
      return { status: resolved.status, reason: `derived_from_exact_sha_repository_check:${repositoryCheck}`, source: resolved.source };
    }
    return { status: STATUS.NOT_VERIFIED, reason: primaryResult.kind === 'missing' ? 'evidence_file_missing' : 'evidence_not_conclusive', source: 'none' };
  };
}
...
export function calculateScorecard(config, evidenceReader = createEvidenceReader()) {
  ...
  const evidenceSources = Object.fromEntries(['configured_evidence', 'repository_ci', 'none', 'custom_reader'].map((source) => [source, controls.filter((item) => item.evidenceSource === source).length]));
  ...
  const publishRecommendation = releaseDecision === 'GO' ? 'ENTERPRISE_PRODUCTION' : scorePercent >= 75 && criticalFailed === 0 ? 'PRODUCTION_WITH_ENTERPRISE_LIMITATIONS' : scorePercent >= 60 && criticalFailed === 0 ? 'CONTROLLED_BETA' : 'DO_NOT_PUBLISH';
  return {
...
export function renderMarkdown(scorecard) {
  return [
    ...
    '',
    ...scorecard.controls.filter((control) => control.critical && control.status !== STATUS.PASS).slice(0, 10).map((control) => `- **${control.id} — ${control.title}:** ${control.status} (${control.evidencePath})`),
    '',
