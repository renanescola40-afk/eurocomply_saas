...
} from '../security/supabase-forward-manifest-contract.mjs';
...
const TENANT_OPERATIONS = ['crossTenantReadDenied', 'crossTenantInsertDenied', 'crossTenantUpdateDenied', 'crossTenantDeleteDenied'];

export function validateSupabaseRlsRuntimeEvidence(
  evidence,
  {
    now = new Date(), maxAgeDays = 7,
    expectedRepository = 'renanescola40-afk/eurocomply_saas', expectedBranch = 'main',
  } = {},
) {
  const failures = [];
  ...
  if (evidence?.schema !== LIVE_RLS_EVIDENCE_SCHEMA) failures.push(`schema must be ${LIVE_RLS_EVIDENCE_SCHEMA}`);
  ...
  if (generatedAt === null) failures.push('generatedAt must be an ISO-8601 timestamp');
    ...
    if (ageMs > maxAgeDays * 86400000) failures.push(`generatedAt is older than ${maxAgeDays} days`);
  }
  ...
  for (const table of REQUIRED_TABLES) {
    ...
    if (matches.length !== 1) { failures.push(`tablesReviewed must contain exactly one ${table} entry`); continue; }
    ...
    if (table === 'regulatory_updates') {
      if (entry.operations?.globalProductBackendOnly !== true) failures.push('regulatory_updates must be backend-only');
      continue;
    }
    for (const operation of TENANT_OPERATIONS) if (entry.operations?.[operation] !== true) failures.push(`${table}.${operation} must be true`);
  ...
  const profileCases = Array.isArray(evidence?.testCases) ? evidence.testCases.filter((test) => test?.table === 'profiles') : [];
  for (const operation of ['rls_enabled', 'cross_tenant_read', 'cross_tenant_insert', 'cross_tenant_update', 'cross_tenant_delete', 'same_tenant_read']) {
    if (profileCases.filter((test) => test?.operation === operation && test?.passed === true).length !== 1) failures.push(`profiles:${operation} must have exactly one passing live test`);
  }
