  ...
  return new Promise((resolve) => {
    ...
    const request = client.request(parsed, {
      ...
      headers: {
        ...
        'User-Agent': 'eurocomply-release-smoke/1.0',
        ...extraHeaders,
      },
    }, (response) => {
      let bodyText = '';
...
async function smoke(baseUrl) {
  ...
  const readyAuthenticated = healthToken
    ? await requestJson(`${baseUrl}/api/ready`, { Authorization: `${['Bear', 'er'].join('')} ${healthToken}` })
    : { status: 0, error: 'missing_protected_readiness_token' };
  const checks = {
    ...
    readyUsesProtectedCheck: readyAuthenticated.body?.checks?.healthcheckProtected === true,
  };
  return {
    baseUrl,
    passed: Object.values(checks).every(Boolean),
    checks,
    health: { status: health.status, bodyStatus: health.body?.status ?? null, error: health.error ?? null },
    readyAnonymous: { status: readyAnonymous.status, bodyStatus: readyAnonymous.body?.status ?? null, error: readyAnonymous.error ?? null },
    readyAuthenticated: { status: readyAuthenticated.status, bodyStatus: readyAuthenticated.body?.status ?? null, error: readyAuthenticated.error ?? null },
  };
}
...
const evidence = {
  ...
  summary: outcome === 'passed' ? 'Deployment smoke passed for /api/health and protected /api/ready.' : 'Deployment smoke is missing or failed; release remains blocked.',
  redactionConfirmation: 'Redaction confirmed for runtime evidence.',
  evidenceLocations: ['scripts/release/run-deployment-smoke.mjs', 'src/app/api/health/route.ts', 'src/app/api/ready/route.ts', evidencePath],
  controlsVerified: outcome === 'passed' ? ['/api/health returned ok.', '/api/ready rejected anonymous access.', '/api/ready returned ready with protected release token.', 'Protected release token value was not printed or stored.'] : [],
  runtimeConfiguration: { targetCount: targetUrls.length, hasProtectedReadinessToken: Boolean(healthToken), timeoutMs },
  targets: targetResults,
  failures,
  releaseGate: outcome === 'passed' ? 'Deployment smoke evidence is present.' : 'Production and enterprise release remain blocked until deployment smoke is Complete/passed.',
