...
function provenanceFailures(env, targetCommit) {
  const failures = [];
  ...
  if (!String(env.GITHUB_RUN_ID ?? '').trim()) failures.push('GITHUB_RUN_ID is required');
  if (!/^[a-f0-9]{40}$/i.test(targetCommit)) failures.push('target commit must be a full 40-character SHA');
  return failures;
}
...
export function buildFinalValidationRunnerEvidence({
  ...
  const provenance = provenanceFailures(env, targetCommit);
  const failures = [
  ...
  return {
    ...
    releaseTarget: env.RELEASE_TARGET || productionFinal?.releaseTarget || 'enterprise',
    targetCommit,
