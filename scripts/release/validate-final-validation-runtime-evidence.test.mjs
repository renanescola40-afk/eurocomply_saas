...
function completeEvidence(overrides = {}) {
  return {
    ...
    summary: 'Canonical final validation completed successfully for the exact enterprise release commit with every required control verified.',
    evidenceLocations: ['docs/security/evidence/runtime/production-final-validation.json'],
    controlsVerified: ['canonical final-validation command set'],
...
describe('validateFinalValidationRuntimeEvidence', () => {
  ...
  it('rejects Complete evidence without P0 file-hygiene fields', () => {
    const evidence = completeEvidence({
      summary: '',
      evidenceLocations: [],
      controlsVerified: [],
      redactionConfirmation: 'custom wording',
    });
    const failures = validateFinalValidationRuntimeEvidence(evidence, { now });

    expect(failures).toContain('summary must be a non-empty string');
    expect(failures).toContain('evidenceLocations must be a non-empty array');
    expect(failures).toContain('controlsVerified must be a non-empty array');
    expect(failures).toContain('redactionConfirmation must use the canonical runtime-evidence text');
  });

  it('writer produces Complete evidence accepted by validator and P0 file contract', () => {
    ...
    expect(evidence.summary.length).toBeGreaterThanOrEqual(40);
    expect(evidence.evidenceLocations.length).toBeGreaterThan(0);
    expect(evidence.controlsVerified.length).toBeGreaterThan(0);
    expect(evidence.redactionConfirmation).toBe(canonicalRedactionConfirmation);
