...
const evidence = {
  ...
  summary: outcome === 'passed' ? 'Rollback dry-run metadata is present. Run deployment smoke against the target before Go.' : 'Rollback dry-run evidence is incomplete; release remains blocked.',
  ...
  controlsVerified: outcome === 'passed' ? ['Previous known-good URL was configured.', 'Previous known-good full commit SHA was configured.', 'Dry-run performed no production mutation.'] : [],
  ...
  dryRun: { mutatesProduction: false, commandMode: 'metadata-only' },
  targetValidation: { passed: false, reason: 'Run scripts/release/run-deployment-smoke.mjs against the rollback target before Go.' },
  failures,
  releaseGate: 'Release remains blocked until rollback dry-run and target validation are Complete/passed.',
  evidenceIntegrity: { containsSensitiveValues: false, valuesRedacted: true },
};
mkdirSync(dirname(evidencePath), { recursive: true });
writeFileSync(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`);
console.log(`Wrote ${evidencePath}`);
if (failures.length > 0 || evidence.targetValidation.passed !== true) {
  console.error('Rollback dry-run validation failed: real target validation is still required.');
  process.exit(1);
}
