...
export function validatePublicProductionGoNoGoEvidence(document, {
  expectedCommitSha,
  ...
  if (document?.schema !== 'risck-comply.public-production-go-no-go.v1') failures.push('schema_invalid');
  if (document?.evidenceItem !== 'release-go-no-go') failures.push('evidence_item_invalid');
  if (document?.profile !== 'public-production') failures.push('profile_invalid');
  ...
  if (!ALLOWED_TARGETS.has(expectedReleaseTarget) || document?.releaseTarget !== expectedReleaseTarget) {
    failures.push('release_target_mismatch');
  }
  ...
  if (document?.evidenceIntegrity?.rawUrlsStored !== false) failures.push('raw_urls_must_not_be_stored');
  if (document?.evidenceIntegrity?.exactReleaseShaRequired !== true) failures.push('exact_release_sha_required');
  if (document?.evidenceIntegrity?.staleEvidenceAccepted !== false) failures.push('stale_evidence_must_not_be_accepted');
...
function runCli() {
  ...
  const expectedReleaseTarget = String(process.env.RELEASE_TARGET || 'public-production').trim().toLowerCase();
  let document;

  try {
    document = JSON.parse(readFileSync(path, 'utf8'));
  } catch (error) {
    console.error(`Public production Go/No-Go evidence is missing or invalid: ${error instanceof Error ? error.message : error}`);
    process.exit(1);
  }

  const failures = validatePublicProductionGoNoGoEvidence(document, {
    expectedCommitSha,
    expectedBuildSha,
    expectedReleaseTarget,
  });
  if (failures.length > 0) {
    console.error('Public production Go/No-Go evidence validation failed:');
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }

  console.log(`Public production Go/No-Go evidence passed for ${expectedCommitSha}.`);
}
