...
const automaticRollbackValidated = rollbackResolutionMode === 'automatic'
...
if (!automaticRollbackValidated
  && !process.env.RELEASE_ROLLBACK_TARGET
  && !process.env.RELEASE_ROLLBACK_TARGET_URL
  && !process.env.LAST_KNOWN_GOOD_DEPLOYMENT_URL) {
  metadataFailures.push('Missing rollback target.');
}
